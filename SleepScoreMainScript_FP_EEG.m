...
function SleepScoreMainScript_FP_EEG()
...
AddSleepParameters_FP_EEG(procDataFileIDs,RestingBaselines,baselineType,NBins)
% create a table of values for sleep scoring model
CreateModelDataSet_FP_EEG(procDataFileIDs,NBins)
% create manual decisions for each 5 second bin
CreateTrainingDataSet_FP_EEG(procDataFileIDs,NBins)
...
for c = 1:length(modelNames)
    ...
    REMsleepTime = 30;   % seconds
    [SleepData] = CreateSleepData_FP_EEG_Auto(startingDirectory,trainingDirectory,baselineDirectory,NREMsleepTime,REMsleepTime,modelName,SleepData);
end 
cd(baselineDirectory)
