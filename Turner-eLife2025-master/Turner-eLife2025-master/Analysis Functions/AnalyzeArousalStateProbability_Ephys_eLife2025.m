...
for ff = 1:length(bins)
    Results_ArousalStateProb_Ephys.(group).(animalID).SleepProbability.(bins{1,ff}).awakeLogical = data.(bins{1,ff}).awakeLogical;
end
...
for aa = 1:size(data.uniqueDays,1)
    Results_ArousalStateProb_Ephys.(group).(animalID).Hypnogram.(data.uniqueDays{aa,1}) = data.(data.uniqueDays{aa,1});
end
% percentage of time in each state
Results_ArousalStateProb_Ephys.(group).(animalID).numberOfScores = length(ScoringResults.alllabels);
Results_ArousalStateProb_Ephys.(group).(animalID).awakePercent = round((sum(strcmp(ScoringResults.alllabels,'Not Sleep'))/length(ScoringResults.alllabels))*100,1);
Results_ArousalStateProb_Ephys.(group).(animalID).nremPercent = round((sum(strcmp(ScoringResults.alllabels,'NREM Sleep'))/length(ScoringResults.alllabels))*100,1);
Results_ArousalStateProb_Ephys.(group).(animalID).remPercent = round((sum(strcmp(ScoringResults.alllabels,'REM Sleep'))/length(ScoringResults.alllabels))*100,1);
% save results
cd([rootFolder delim 'Results_Turner'])
save('Results_ArousalStateProb_Ephys.mat','Results_ArousalStateProb_Ephys')
