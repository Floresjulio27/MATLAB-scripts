function [Results_PowerSpec_2P] = AnalyzePowerSpectrum_2P_eLife2025(animalID,group,set,rootFolder,delim,Results_PowerSpec_2P)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:size(mergedDataFileIDs,1)
    ...
    [~,~,fileDate,~,~,vesselID] = GetFileInfo2_IOS_eLife2025(mergedDataFileID);
    if strcmp(vesselID(1),'V') == false
        ...
        [linkedBinarizedWhiskers] = LinkBinaryEvents_IOS_eLife2025(gt(binWhisk,0),[round(30/3),0]);
...
if isempty(allData) == false
    ...
    for bb = 1:length(allDataVesselIDs)
        ...
        % save results
        Results_PowerSpec_2P.(group).(animalID).(allDataVID).whiskingPerc = mean(binWhisking.(allDataVID));
        Results_PowerSpec_2P.(group).(animalID).(allDataVID).S = allData_S{bb,1};
        Results_PowerSpec_2P.(group).(animalID).(allDataVID).f = allData_f{bb,1};
        Results_PowerSpec_2P.(group).(animalID).(allDataVID).sErr = allData_sErr{bb,1};
    end
end
% save data
cd([rootFolder delim 'Results_Turner'])
save('Results_PowerSpec_2P.mat','Results_PowerSpec_2P')
