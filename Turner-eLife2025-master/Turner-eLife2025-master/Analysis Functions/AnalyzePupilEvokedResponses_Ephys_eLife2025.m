function [Results_PupilEvoked_Ephys] = AnalyzePupilEvokedResponses_Ephys_eLife2025(animalID,group,set,rootFolder,delim,Results_PupilEvoked_Ephys)
%----------------------------------------------------------------------------------------------------------
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(WhiskCriteriaNames)
        ...
        [finalWhiskData,~,~,~] = RemoveInvalidData_IOS_eLife2025(allWhiskData,allWhiskFileIDs,allWhiskDurations,allWhiskEventTimes,ManualDecisions);
    ...
    for gg = 1:length(stimCriteriaNames)
        ...
        % pull data from EventData.mat structure
        allStimFilter = FilterEvents_IOS_eLife2025(EventData.Pupil.(dataType).stim,StimCriteria);
        ...
        % keep only the data that occurs within the manually-approved awake regions
        [finalStimData,~,~,~] = RemoveInvalidData_IOS_eLife2025(allStimData,allStimFileIDs,allStimDurations,allStimEventTimes,ManualDecisions);
