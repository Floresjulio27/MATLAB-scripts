...
for aa = 1:iterations
    ...
    totalScores = sum(confVals([3,6,9]));
    modelAccuracy = (sum(confVals([9])/totalScores))*100;
...
Results_ModelAccuracy_GCaMP.(group).(animalID).mdl = bestMDL;
Results_ModelAccuracy_GCaMP.(group).(animalID).outOfBagError = outOfBagError;
Results_ModelAccuracy_GCaMP.(group).(animalID).trueTestingLabels = physioYtesting.behavState;
Results_ModelAccuracy_GCaMP.(group).(animalID).predictedTestingLabels = physioTestingPredictions;
% save results
