...
if runFromStart == true
        Results_Diameter_2P.Blank_SAP = [];
        Results_Diameter_2P.SSP_SAP = [];
elseif runFromStart == false
    cd([rootFolder delim 'Results_Turner']);
    % load existing results structure, if it exists
    if exist('Results_Diameter_2P.mat','file') == 2
        load('Results_Diameter_2P.mat','-mat')
    else
        Results_Diameter_2P.Blank_SAP = [];
        Results_Diameter_2P.SSP_SAP = [];
    end
...
cc = 1;
multiWaitbar('Analyzing arteriole diameter for 2P',0,'Color','P');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
