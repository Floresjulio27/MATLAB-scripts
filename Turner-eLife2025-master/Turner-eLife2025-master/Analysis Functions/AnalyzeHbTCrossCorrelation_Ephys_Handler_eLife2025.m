...
if runFromStart == true
    Results_HbTCrossCorr_Ephys.Naive = [];
    Results_HbTCrossCorr_Ephys.Blank_SAP = [];
    Results_HbTCrossCorr_Ephys.SSP_SAP = [];
elseif runFromStart == false
    cd([rootFolder delim 'Results_Turner']);
    % load existing results structure, if it exists
    if exist('Results_HbTCrossCorr_Ephys.mat','file') == 2
        load('Results_HbTCrossCorr_Ephys.mat','-mat')
    else
        Results_HbTCrossCorr_Ephys.Naive = [];
        Results_HbTCrossCorr_Ephys.Blank_SAP = [];
        Results_HbTCrossCorr_Ephys.SSP_SAP = [];
