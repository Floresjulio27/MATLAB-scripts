function [Results_NeuralHemoCoher_Ephys] = AnalyzeNeuralHemoCoherence_Ephys_eLife2025(animalID,group,set,rootFolder,delim,Results_NeuralHemoCoher_Ephys)
%----------------------------------------------------------------------------------------------------------
...
for zzz = 1:length(hemispheres)
    ...
    for xx = 1:length(dataTypes)
        ...
        %% NREM
        [HbT_nremData,~,~] = RemoveStimSleepData_IOS_eLife2025(animalID,SleepData.(modelType).NREM.data.HbT.(hemisphere),SleepData.(modelType).NREM.FileIDs,SleepData.(modelType).NREM.BinTimes);
        [Neural_nremData,~,~] = RemoveStimSleepData_IOS_eLife2025(animalID,SleepData.(modelType).NREM.data.(['cortical_' hemisphere]).(dataType),SleepData.(modelType).NREM.FileIDs,SleepData.(modelType).NREM.BinTimes);
        % filter, detrend, and truncate data to minimum length to match events
        ...
        %% REM
        [HbT_remData,~,~] = RemoveStimSleepData_IOS_eLife2025(animalID,SleepData.(modelType).REM.data.HbT.(hemisphere),SleepData.(modelType).REM.FileIDs,SleepData.(modelType).REM.BinTimes);
        [Neural_remData,~,~] = RemoveStimSleepData_IOS_eLife2025(animalID,SleepData.(modelType).REM.data.(['cortical_' hemisphere]).(dataType),SleepData.(modelType).REM.FileIDs,SleepData.(modelType).REM.BinTimes);
        % filter, detrend, and truncate data to minimum length to match events
