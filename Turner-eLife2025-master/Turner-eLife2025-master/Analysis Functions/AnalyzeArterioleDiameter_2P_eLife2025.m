function [Results_Diameter_2P] = AnalyzeArterioleDiameter_2P_eLife2025(animalID,group,set,rootFolder,delim,Results_Diameter_2P)
%----------------------------------------------------------------------------------------------------------
...
[sos,g] = zp2sos(z,p,k);
%% Rest
[restLogical] = FilterEvents_2P_eLife2025(RestData.vesselDiameter.data,RestCriteria);
...
[finalRestVesselData,finalRestFileIDs,finalRestVesselIDs,~,~] = RemoveInvalidData_2P_eLife2025(restVesselData,restFileIDs,restVesselIDs,restDurations,restEventTimes,ManualDecisions);
...
for aa = 1:length(uniqueRestVesselIDs)
    ...
    for bb = 1:length(finalRestVesselIDs)
        if strcmp(uniqueRestVesselIDs{aa,1},finalRestVesselIDs{bb,1})
            strDay = ConvertDate_2P_eLife2025(finalRestFileIDs{bb,1}(1:6));
...
% keep only the data that occurs within the manually-approved awake regions
[finalWhiskVesselData,finalWhiskFileIDs,finalWhiskVesselIDs,~,~] = RemoveInvalidData_2P_eLife2025(whiskVesselData,whiskFileIDs,whiskVesselIDs,whiskDurations,whiskEventTimes,ManualDecisions);
...
for aa = 1:length(uniqueWhiskVesselIDs)
    ...
    for bb = 1:length(finalWhiskVesselIDs)
        if strcmp(uniqueWhiskVesselIDs{aa,1},finalWhiskVesselIDs{bb,1})
            strDay = ConvertDate_2P_eLife2025(finalWhiskFileIDs{bb,1}(1:6));
