function [] = AnalyzeCrossCorrelation_Ephys_Handler_eLife2025(rootFolder,delim,runFromStart)
%----------------------------------------------------------------------------------------------------------
...
cc = 1;
multiWaitbar('Analyzing cross correlation for Ephys',0,'Color','P');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
    ...
    for bb = 1:length(animalIDs)
        if isfield(Results_CrossCorr_Ephys.(groups{1,aa}),(animalIDs{1,bb})) == false
            [Results_CrossCorr_Ephys] = AnalyzeCrossCorrelation_Ephys_eLife2025(animalIDs{1,bb},groups{1,aa},set,rootFolder,delim,Results_CrossCorr_Ephys);
        end
