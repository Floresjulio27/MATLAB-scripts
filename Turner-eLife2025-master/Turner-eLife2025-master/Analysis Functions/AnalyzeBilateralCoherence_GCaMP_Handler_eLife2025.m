function [] = AnalyzeBilateralCoherence_GCaMP_Handler_eLife2025(rootFolder,delim,runFromStart)
%----------------------------------------------------------------------------------------------------------
...
cc = 1;
multiWaitbar('Analyzing bilateral coherence for GCaMP',0,'Color','P');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
    animalIDs = {folderList.name};
    for bb = 1:length(animalIDs)
        if isfield(Results_BilatCoher_GCaMP.(groups{1,aa}),(animalIDs{1,bb})) == false
            [Results_BilatCoher_GCaMP] = AnalyzeBilateralCoherence_GCaMP_eLife2025(animalIDs{1,bb},groups{1,aa},set,rootFolder,delim,Results_BilatCoher_GCaMP);
        end
