...
if runFromStart == true
        Results_PupilBlinkInterval_Ephys.Naive = [];
        Results_PupilBlinkInterval_Ephys.Blank_SAP = [];
        Results_PupilBlinkInterval_Ephys.SSP_SAP = [];
elseif runFromStart == false
    cd([rootFolder delim 'Results_Turner']);
    % load existing results structure, if it exists
    if exist('Results_PupilBlinkInterval_Ephys.mat','file') == 2
        load('Results_PupilBlinkInterval_Ephys.mat','-mat')
    else
        Results_PupilBlinkInterval_Ephys.Naive = [];
        Results_PupilBlinkInterval_Ephys.Blank_SAP = [];
        Results_PupilBlinkInterval_Ephys.SSP_SAP = [];
...
cc = 1;
multiWaitbar('Analyzing pupil inter-blink interval for Ephys',0,'Color','P');
for aa = 1:length(groups)
    folderList = dir([groups{1,aa} delim set]);
    folderList = folderList(~startsWith({folderList.name},'.'));
