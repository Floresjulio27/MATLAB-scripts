    ...
    for bb = 1:size(procDataFileIDs,1)
        ...
        if sum(strcmp(scoringLabels,'Not Sleep')) > 144   % 36 bins (180 total) or 3 minutes of sleep
            ...
            if isempty(puffs) == true
                NeuralData{dd,1} = (ProcData.data.(['cortical_' hemisphere]).(neuralBand) - RestingBaselines.manualSelection.(['cortical_' hemisphere]).(neuralBand).(strDay).mean)./RestingBaselines.manualSelection.(['cortical_' hemisphere]).(neuralBand).(strDay).mean; %#ok<*AGROW>
...
elseif strcmp(behavior,'Asleep') == true
    ...
    for ee = 1:size(procDataFileIDs,1)
        ...
        if sum(strcmp(scoringLabels,'Not Sleep')) < 36   % 36 bins (180 total) or 3 minutes of awake
            ...
            % don't include trials with stimulation
            if isempty(puffs) == true
                NeuralData{gg,1} = (ProcData.data.(['cortical_' hemisphere]).(neuralBand) - RestingBaselines.manualSelection.(['cortical_' hemisphere]).(neuralBand).(strDay).mean)./RestingBaselines.manualSelection.(['cortical_' hemisphere]).(neuralBand).(strDay).mean; %#ok<*AGROW>
