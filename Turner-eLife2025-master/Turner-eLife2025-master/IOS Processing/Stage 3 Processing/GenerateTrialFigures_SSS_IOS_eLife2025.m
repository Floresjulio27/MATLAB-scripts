function [figHandle] = GenerateTrialFigures_SSS_IOS_eLife2025(procDataFileID,RestingBaselines)
...
if any(strcmp(imagingWavelengths,{'Red, Green, & Blue','Lime, Green, & Blue'})) == true
    ...
    % GCaMP
    sss_GCaMP7s = ProcData.data.GCaMP7s.corSSS;
    normSSS_GCaMP7s = (sss_GCaMP7s - 1).*100;
    filtSSS_GCaMP7s = filtfilt(sos2,g2,normSSS_GCaMP7s);
    % Deoxy signal
    sss_deoxy = ProcData.data.Deoxy.SSS;
    normSSS_Deoxy = ((sss_deoxy - RestingBaselines.manualSelection.Deoxy.SSS.(strDay).mean)./RestingBaselines.manualSelection.Deoxy.SSS.(strDay).mean)*100;
    filtSSS_Deoxy = filtfilt(sos2,g2,normSSS_Deoxy);
...
elseif strcmp(imagingWavelengths,'Blue') == true
    ...
    normSSS_CBV = (sss_CBV - RestingBaselines.manualSelection.CBV.SSS.(strDay).mean)./(RestingBaselines.manualSelection.CBV.SSS.(strDay).mean);
    filtSSS_CBV = (filtfilt(sos2,g2,normSSS_CBV)).*100;
    % L dR/R
    lsss_CBV = ProcData.data.CBV.lSSS;
    normlSSS_CBV = (lsss_CBV - RestingBaselines.manualSelection.CBV.lSSS.(strDay).mean)./(RestingBaselines.manualSelection.CBV.lSSS.(strDay).mean);
    filtlSSS_CBV = (filtfilt(sos2,g2,normlSSS_CBV)).*100;
    % R dR/R
    rsss_CBV = ProcData.data.CBV.rSSS;
    normrSSS_CBV = (rsss_CBV - RestingBaselines.manualSelection.CBV.rSSS.(strDay).mean)./(RestingBaselines.manualSelection.CBV.rSSS.(strDay).mean);
    filtrSSS_CBV = (filtfilt(sos2,g2,normrSSS_CBV)).*100;
end
...
if any(strcmp(imagingWavelengths,{'Red, Green, & Blue','Lime, Green, & Blue'})) == true
    ...
    % GCaMP
    ax4 = subplot(8,1,4);
    plot((1:length(filtSSS_GCaMP7s))/ProcData.notes.wavelengthSamplingRate,filtSSS_GCaMP7s,'color',colors('dark candy apple red'),'LineWidth',1);
    ...
    % Deoxy
    ax5 = subplot(8,1,5);
    plot((1:length(filtSSS_Deoxy))/ProcData.notes.wavelengthSamplingRate,filtSSS_Deoxy,'color',colors('dark candy apple red'),'LineWidth',1);
...
elseif strcmp(imagingWavelengths,'Blue') == true
    ...
    s5 = scatter(AudSol,Aud_Yvals,'v','MarkerEdgeColor','k','MarkerFaceColor','g');
    s6 = scatter(OptoLED,Opto_Yvals,'v','MarkerEdgeColor','k','MarkerFaceColor','b');
    p5 = plot((1:length(filtlSSS_CBV))/ProcData.notes.wavelengthSamplingRate,filtlSSS_CBV,'color',colors('red'),'LineWidth',1);
    p6 = plot((1:length(filtSSS_CBV))/ProcData.notes.wavelengthSamplingRate,filtSSS_CBV,'color',colors('green'),'LineWidth',1);
    p7 = plot((1:length(filtrSSS_CBV))/ProcData.notes.wavelengthSamplingRate,filtrSSS_CBV,'color',colors('blue'),'LineWidth',1);
    legend([p5,p6,p7,s1,s2,s3,s4,s5,s6],'lSSS','SSS','rSSS','movement','whisking',',LPad sol','RPad sol','Aud sol','Opto LED')
    ylabel('\DeltaF/F')
    xlim([0,ProcData.notes.trialDuration_sec])
    set(gca,'TickLength',[0,0])
    set(gca,'Xticklabel',[])
