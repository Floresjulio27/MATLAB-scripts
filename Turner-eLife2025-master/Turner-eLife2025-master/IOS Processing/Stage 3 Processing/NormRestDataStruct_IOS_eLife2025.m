function [RestData] = NormRestDataStruct_IOS_eLife2025(animalID,RestData,RestingBaselines,baselineType)
%----------------------------------------------------------------------------------------------------------
...
for dT = 1:length(dataTypes)
    ...
    for hDT = 1:length(hemisphereDataTypes)
        ...
        normData = {};
        [uniqueDays,~,~] = GetUniqueDays_IOS_eLife2025(RestData.(dataType).(hemDataType).fileDates);
        for uD = 1:length(uniqueDays)
            date = uniqueDays{uD};
            strDay = ConvertDate_IOS_eLife2025(date);
            [~,dayInds] = GetDayInds_IOS_eLife2025(RestData.(dataType).(hemDataType).fileDates,date);
