function [RestingBaselines,ManualDecisions,procDataFileIDs,animalID] = CalculateManualRestingBaselinesTimeIndeces_IOS_eLife2025(hemoType)
%----------------------------------------------------------------------------------------------------------
...
for a = 1:size(procDataFileIDs,1)
    ...
    [animalID,~,~] = GetFileInfo_IOS_eLife2025(procDataFileID);
    ...
    if isfield(ProcData,'manualBaselineInfo') == false
        ...
        while b == false
            ...
            [singleTrialFig] = GenerateSingleFigures_IOS_eLife2025(procDataFileID,RestingBaselines,'setDuration',hemoType);
...
for e = 1:length(dataTypes)
    ...
    if strcmp(dataType,'Pupil') == false
        ...
        for f = 1:length(subDataTypes)
            ...
            [restLogical] = FilterEvents_IOS_eLife2025(RestData.(dataType).(subDataType),RestCriteria);
            [stimLogical] = FilterEvents_IOS_eLife2025(RestData.(dataType).(subDataType),StimCriteria);
            ...
            uniqueDays = GetUniqueDays_IOS_eLife2025(RestData.(dataType).(subDataType).fileIDs);
            ...
            for p = 1:length(uniqueDays)
                ...
                for r = 1:length(finalEventFileIDs)
                    ...
                    uniqueDate{p,1} = ConvertDate_IOS_eLife2025(uniqueDays{p,1});
