function [RestingBaselines] = CalculateRestingBaselines_IOS_eLife2025(RestData,targetMinutes)
%----------------------------------------------------------------------------------------------------------
...
for a = 1:length(dataTypes)
    ...
    for b = 1:length(subDataTypes)
        ...
        [restLogical] = FilterEvents_IOS_eLife2025(RestData.(dataType).(subDataType),RestCriteria); % output is a logical
        [stimLogical] = FilterEvents_IOS_eLife2025(RestData.(dataType).(subDataType),stimCriteria); % output is a logical
        ...
        uniqueDays = GetUniqueDays_IOS_eLife2025(RestData.(dataType).(subDataType).fileIDs); % find the unique days of imaging
        ...
        for e = 1:length(uniqueDays)
            ...
            for f = 1:length(finalFileIDs)
                ...
                date{e,1} = ConvertDate_IOS_eLife2025(uniqueDays{e,1});
