function [EventData] = ExtractEventTriggeredData_2P_eLife2025(mergedDataFileIDs,dataTypes)
...
for dT = 1:length(dataTypes)
    ...
    for f = 1:size(mergedDataFileIDs,1)
        ...
        [animalID,~,fileDate,fileID,imageID,vesselID] = GetFileInfo2_2P_eLife2025(mergedDataFileIDs(f,:));
        ...
        for c = 1:length(subDataTypes)
            ...
            for bF = 1:length(behaviorFields)
                ...
                [chunkData,evFilter] = ExtractBehavioralData_2P_eLife2025(data,epoch,sDT,samplingRate,behaviorFields{bF});
                % Add epoch details to temp struct
                [temp] = AddEpochInfo_2P_eLife2025(data,sDT,behaviorFields{bF},temp,fileID,fileDate,imageID,vesselID,evFilter,f);
                temp.(sDT).(behaviorFields{bF}).data{f} = chunkData;
            end
        end
    end
    % Convert the temporary stuct into a final structure
    [EventData] = ProcessTempStruct_2P_eLife2025(EventData,dataType,temp,epoch);
end
save([animalID '_EventData.mat'],'EventData');

end

function [chunkData,evFilter] = ExtractBehavioralData_2P_eLife2025(data,epoch,dataType,samplingRate,behavior)
% Setup variables
...
function [temp] = AddEpochInfo_2P_eLife2025(data,dataType,behavior,temp,fileID,fileDate,imageID,vesselID,evFilter,f)
...
function [EventData] = ProcessTempStruct_2P_eLife2025(EventData,dataType,temp,epoch)
