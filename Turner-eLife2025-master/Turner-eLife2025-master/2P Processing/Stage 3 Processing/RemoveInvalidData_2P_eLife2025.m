function [decData,decFileIDs,decVesselIDs,decDurations,decEventTimes] = RemoveInvalidData_2P_eLife2025(data,fileIDs,vesselIDs,durations,eventTimes,ManualDecisions)
...
for a = 1:size(data,1)
    ...
    for b = 1:length(ManualDecisions.fileIDs)
        [~,~,~,manualFileID,~,~] = GetFileInfo2_2P_eLife2025(ManualDecisions.fileIDs{b,1});
    ...
    if startTime >= manualStartTime && endTime <= manualEndTime
        if startTime >= offset && endTime <= (trialDuration_sec - offset)
            ...
            decVesselIDs{x,1} = vesselIDs{a,1};
