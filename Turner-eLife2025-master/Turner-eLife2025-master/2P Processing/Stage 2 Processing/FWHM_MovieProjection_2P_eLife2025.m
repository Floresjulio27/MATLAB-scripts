function [MScanData] = FWHM_MovieProjection_2P_eLife2025(MScanData,theFrames)
...
for f = min(theFrames):max(theFrames)
    ...
    MScanData.data.rawVesselDiameter(f) = CalcFWHM_2P_eLife2025(medfilt1(MScanData.notes.vesselROI.projection(f,:),5));
end
