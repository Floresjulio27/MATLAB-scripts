function [] = MainScript_FP_GRABNE_Duplicate()
...
clear; clc; close all;
%% Animal IDs
FP_animalIDs =    {'NEACh009','NEACh010','NEACh011','NEAChGi001','NEAChGi002','NEAChChR2002'};% 
...
[AnalysisResults] = Fig1_S2_Stim_GRABNE_Response_CI_Flavoprotein(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FP_animalIDs);
...
% [AnalysisResults] = Fig1_S3_Whisk_Flavoprotein_Response_CI(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FP_animalIDs);
% [AnalysisResults] = Fig1_S3_Whisk_Flavoprotein_Response_CI_Flavoprotein(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FP_animalIDs);
...
% [AnalysisResults] = Fig4_FP_Transition_Flavoprotein_SingleMouse_consolidated_CI(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FP_animalIDs);
...
for dd = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,dd})) == false || isfield(AnalysisResults.(FP_animalIDs{1,dd}),'Transitions') == false || strcmp(runFromStart,'y') == true
          ...
          % [AnalysisResults] = AnalyzeTransitionalAverages_FP_Pupil(FP_animalIDs{1,dd},saveFigs,rootFolder,AnalysisResults,firstHrs);
...
for pp = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,pp})) == false || isfield(AnalysisResults.(FP_animalIDs{1,pp}),'Movement') == false ||isfield(AnalysisResults.(FP_animalIDs{1,pp}),'Whisk') == false || isfield(AnalysisResults.(FP_animalIDs{1,pp}),'Stim') == false ||strcmp(runFromStart,'y') == true
        ...
        % [AnalysisResults] = AnalyzeEvokedResponses_GRABNE_Sleep(FP_animalIDs{1,pp},rootFolder,AnalysisResults,firstHrs);
        % [AnalysisResults] = AnalyzeEvokedResponses_GRABNE_Sleep_Flavoprotein(FP_animalIDs{1,pp},rootFolder,AnalysisResults,firstHrs);
