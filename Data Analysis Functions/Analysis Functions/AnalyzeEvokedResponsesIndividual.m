function [Results_Evoked] = AnalyzeEvokedResponsesIndividual(animalID,rootFolder,Results_Evoked)
...
%     end
    ...
    samplingRate = EventData.TRITC.(dataType).whisk.samplingRate;
    specSamplingRate = 10;
    trialDuration_sec = EventData.TRITC.(dataType).whisk.trialDuration_sec;
    timeVector = (0:(EventData.TRITC.(dataType).whisk.epoch.duration*samplingRate))/samplingRate - EventData.TRITC.(dataType).whisk.epoch.offset;
    offset = EventData.TRITC.(dataType).whisk.epoch.offset;
    for bb = 1:length(WhiskCriteriaNames)
        ...
        [whiskLogical] = FilterEvents_IOS(EventData.TRITC.(dataType).whisk,WhiskCriteria);
        combWhiskLogical = logical(whiskLogical);
        [allWhiskTRITCData] = EventData.TRITC.(dataType).whisk.NormData(combWhiskLogical,:);
        [allWhiskGCaMP7sData] = EventData.GCaMP7s.(dataType).whisk.NormData(combWhiskLogical,:);% EventData.GCaMP7s.(dataType).whisk.NormData(combWhiskLogical,:);
        ...
        [allWhiskFileIDs] = EventData.TRITC.(dataType).whisk.fileIDs(combWhiskLogical,:);
        [allWhiskEventTimes] = EventData.TRITC.(dataType).whisk.eventTime(combWhiskLogical,:);
        allWhiskDurations = EventData.TRITC.(dataType).whisk.duration(combWhiskLogical,:);
        % keep only the data that occurs within the manually-approved awake regions
        [finalWhiskTRITCData,finalWhiskFileIDs,~,finalWhiskFileEventTimes] = RemoveInvalidData_IOS(allWhiskTRITCData,allWhiskFileIDs,allWhiskDurations,allWhiskEventTimes,ManualDecisions);
        ...
        clear procWhiskGCaMP7sData procWhiskTRITCData procWhiskCorticalMUAData procWhiskHippocampalMUAData procWhiskCorticalGamData procWhiskHippocampalGamData finalWhiskStartTimes finalWhiskEndTimes finalWhiskFiles
        dd = 1;
        for cc = 1:size(finalWhiskTRITCData,1)
            ...
            if whiskStartTime >= 0.5 && whiskEndTime <= (trialDuration_sec - 0.5)
                whiskTRITCarray = finalWhiskTRITCData(cc,:);
                ...
                filtWhiskTRITCarray = sgolayfilt(whiskTRITCarray,3,17);
                filtWhiskGCaMP7sarray = sgolayfilt(whiskGCaMP7sarray,3,17);

                procWhiskTRITCData(dd,:) = filtWhiskTRITCarray- mean(filtWhiskTRITCarray(1:(offset*samplingRate))); %#ok<*AGROW>
        ...
        meanWhiskTRITCData = mean(procWhiskTRITCData,1);%*100;
        stdWhiskTRITCData = std(procWhiskTRITCData,0,1);%*100;
        ...
        % save results
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).TRITC.TRITC = meanWhiskTRITCData;
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).TRITC.TRITCStD = stdWhiskTRITCData;
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).GCaMP7s.GCaMP7s = meanWhiskGCaMP7sData;
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).GCaMP7s.GCaMP7sStD = stdWhiskGCaMP7sData;
        ...
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).LFP.corticalS = msStimWhiskS_Vals;
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).LFP.hippocampalS = msWhiskHipS_Vals;
    ...
    for gg = 1:length(stimCriteriaNames)
        ...
        % pull data from EventData.mat structure
        allStimFilter = FilterEvents_IOS(EventData.TRITC.(dataType).stim,StimCriteria);
        [allStimTRITCData] = EventData.TRITC.(dataType).stim.NormData(allStimFilter,:);
        ...
        [allStimFileIDs] = EventData.TRITC.(dataType).stim.fileIDs(allStimFilter,:);
        [allStimEventTimes] = EventData.TRITC.(dataType).stim.eventTime(allStimFilter,:);
        allStimDurations = zeros(length(allStimEventTimes),1);
        % keep only the data that occurs within the manually-approved awake regions
        [finalStimTRITCData,finalStimFileIDs,~,finalStimFileEventTimes] = RemoveInvalidData_IOS(allStimTRITCData,allStimFileIDs,allStimDurations,allStimEventTimes,ManualDecisions);
        ...
        clear procStimGCaMP7sData procStimTRITCData procStimTRITCData procStimCortMUAData procStimHipMUAData procStimCortGamData procStimHipGamData finalStimStartTimes finalStimEndTimes finalStimFiles
        ii = 1;
        for hh = 1:size(finalStimTRITCData,1)
            ...
            if stimStartTime >= 0.5 && stimEndTime <= (trialDuration_sec - 0.5)
                stimTRITCarray = finalStimTRITCData(hh,:);
                ...
                filtStimTRITCarray = sgolayfilt(stimTRITCarray,3,17);
                filtStimGCaMP7sarray = sgolayfilt(stimGCaMP7sarray,3,17);

                procStimTRITCData(hh,:) = filtStimTRITCarray- mean(filtStimTRITCarray(1:(offset*samplingRate)));
        ...
        meanStimTRITCData = mean(procStimTRITCData,1);%*100;
        stdStimTRITCData = std(procStimTRITCData,0,1);%*100;
        ...
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).TRITC.TRITC = meanStimTRITCData;
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).TRITC.TRITCStD = stdStimTRITCData;
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).GCaMP7s.GCaMP7s= meanStimGCaMP7sData;
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).GCaMP7s.GCaMP7sStD = stdStimGCaMP7sData;
        ...
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).LFP.corticalS = msStimCortS_Vals;
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).LFP.hippocampalS = msStimHipS_Vals;
