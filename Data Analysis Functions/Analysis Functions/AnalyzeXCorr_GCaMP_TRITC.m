function [AnalysisResults] = AnalyzeXCorr_GCaMP_TRITC(animalID,rootFolder,AnalysisResults)
...
for aa = 1:length(dataTypes)
    ...
    [restFinalRestGCaMP7sData,~,~,~] = RemoveInvalidData_IOS(restingGCaMP7sData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
    cc = 1;
...
% 
%                 [restTRITCvGCaMP7sxcVals(ee,:),restLFP_lags] = xcorr(restTRITCarray,restNeuralArray,restMaxLag,'coeff');
%             end
            [restTRITCvMUAxcVals(dd,:),restGCaMP7s_lags] = xcorr(restTRITCarray,restGCaMP7sarray,restMaxLag,'coeff'); %#ok<*AGROW>
            restTRITCvGCaMP7szhold = cat(3,restTRITCvGCaMP7szhold,restTRITCvGCaMP7sxcVals);
        end
        restMeanTRITCvLFPxcVals = mean(restTRITCvGCaMP7szhold,3);
    ...
    AnalysisResults.(animalID).XCorr.Rest.(dataType).MUA_lags = restGCaMP7s_lags;
