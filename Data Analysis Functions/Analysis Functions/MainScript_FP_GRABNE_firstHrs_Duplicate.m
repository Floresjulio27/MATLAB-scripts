function [] = MainScript_FP_GRABNE_firstHrs_Duplicate()
...
%% Animal ID
FP_animalIDs =   {'NEACh012'};%,'NEACh013'}; % {'NEAChGi001','NEAChGi002'};% {'NEAChChR2002'};% ,'NEAChGi001', 'NEAChGi002' ,'NEACh008',
...
% [AnalysisResults_firstHrs] = Fig1_S2_Stim_GRABNE_Response_CI(rootFolder,saveFigs,delim,AnalysisResults_firstHrs,firstHrs,FP_animalIDs);
[AnalysisResults_firstHrs] = Fig1_S2_Stim_GRABNE_Response_CI_Flavoprotein(rootFolder,saveFigs,delim,AnalysisResults_firstHrs,firstHrs,FP_animalIDs);
...
for pp = 1:length(FP_animalIDs)
    if isfield(AnalysisResults_firstHrs,(FP_animalIDs{1,pp})) == false || isfield(AnalysisResults_firstHrs.(FP_animalIDs{1,pp}),'Whisk') == false || isfield(AnalysisResults_firstHrs.(FP_animalIDs{1,pp}),'Stim') == false ||strcmp(runFromStart,'y') == true
        ...
        % [AnalysisResults_firstHrs] = AnalyzeEvokedResponses_GRABNE_Flavoprotein(FP_animalIDs{1,pp},rootFolder,AnalysisResults_firstHrs,firstHrs);

        % [AnalysisResults_firstHrs] = AnalyzeEvokedResponses_GRABNE_Sleep(FP_animalIDs{1,pp},rootFolder,AnalysisResults_firstHrs,firstHrs);
