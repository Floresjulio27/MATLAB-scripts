function [AnalysisResults] = GCaMPvsGRAB_test(rootFolder, saveFigs, delim, AnalysisResults, firstHrs, FPanimalIDs)
...
% Loop through each animal's directory and extract the appropriate analysis results
for aa = 1:length(FPanimalIDs)
    animalID = FPanimalIDs{1, aa};
    for dd = 1:length(solenoidNames)
        solenoidName = solenoidNames{1, dd};
        % Extracting counts
        data.(animalID).P_NE.(solenoidName).count = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).count;
        % Extracting mean CBV and GFP data
        data.(animalID).P_NE.(solenoidName).CBV = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).CBV.CBV;
        data.(animalID).P_ACh.(solenoidName).CBV = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).CBV.CBV;
        data.(animalID).P_NE.(solenoidName).GFP = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).GFP.GFP;
        data.(animalID).P_ACh.(solenoidName).GFP = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).GFP.GFP;
        % Extracting standard deviations
        data.(animalID).P_NE.(solenoidName).CBV_std = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).CBV.CBVStD;
        data.(animalID).P_ACh.(solenoidName).CBV_std = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).CBV.CBVStD;
        data.(animalID).P_NE.(solenoidName).GFP_std = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).GFP.GFPStD;
        data.(animalID).P_ACh.(solenoidName).GFP_std = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).GFP.GFPStD;
        % Extracting neural activity data
        data.(animalID).cortical.(solenoidName).cortMUA = AnalysisResults.(animalID).Stim.cortical.(solenoidName).MUA.corticalData;
        data.(animalID).cortical.(solenoidName).cortGam = AnalysisResults.(animalID).Stim.cortical.(solenoidName).Gam.corticalData;
        data.(animalID).cortical.(solenoidName).timeVector = AnalysisResults.(animalID).Stim.cortical.(solenoidName).timeVector;
        data.(animalID).cortical.(solenoidName).cortS = AnalysisResults.(animalID).Stim.cortical.(solenoidName).LFP.corticalS;
        data.(animalID).cortical.(solenoidName).cortS_Gam = AnalysisResults.(animalID).Stim.cortical.(solenoidName).LFP.corticalS(49:end, 20:23);
        data.(animalID).cortical.(solenoidName).T = AnalysisResults.(animalID).Stim.cortical.(solenoidName).LFP.T;
        data.(animalID).cortical.(solenoidName).F = AnalysisResults.(animalID).Stim.cortical.(solenoidName).LFP.F;
    end
end

% Loop through each animal's directory again for additional data processing
for aa = 1:length(FPanimalIDs)
    animalID = FPanimalIDs{1, aa};
    for dd = 1:length(solenoidNames)
        solenoidName = solenoidNames{1, dd};
        ...
        % Concatenate the raw CBV and GFP data
        data.(animalID).P_NE.(solenoidName).CBVRaw = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).CBV.CBVRaw;
        data.(animalID).P_ACh.(solenoidName).CBVRaw = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).CBV.CBVRaw;
        data.(animalID).P_NE.(solenoidName).GFPRaw = AnalysisResults.(animalID).Stim.P_NE.(solenoidName).GFP.GFPRaw;
        data.(animalID).P_ACh.(solenoidName).GFPRaw = AnalysisResults.(animalID).Stim.P_ACh.(solenoidName).GFP.GFPRaw;
    end
end
...
if strcmp(saveFigs,'y') == true
    ...
    print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_'  'Stim-FiberSignals_ResponseIndividual_CI_SST']) % animalID _NoSleep NoSleep_
    close 
end
