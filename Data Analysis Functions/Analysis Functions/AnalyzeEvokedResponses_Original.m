function [Results_Evoked] = AnalyzeEvokedResponses_Original(animalID,rootFolder,Results_Evoked)
...
%     end
    ...
    samplingRate = EventData.HbT.(dataType).whisk.samplingRate;
    specSamplingRate = 10;
    trialDuration_sec = EventData.HbT.(dataType).whisk.trialDuration_sec;
    timeVector = (0:(EventData.HbT.(dataType).whisk.epoch.duration*samplingRate))/samplingRate - EventData.HbT.(dataType).whisk.epoch.offset;
    offset = EventData.HbT.(dataType).whisk.epoch.offset;
    for bb = 1:length(WhiskCriteriaNames)
        ...
        [whiskLogical] = FilterEvents_IOS(EventData.HbT.(dataType).whisk,WhiskCriteria);
        combWhiskLogical = logical(whiskLogical);
        [allWhiskHbTData] = EventData.HbT.(dataType).whisk.data(combWhiskLogical,:);
        [allWhiskGCaMP7sData] = EventData.GCaMP7s.(dataType).whisk.data(combWhiskLogical,:);% EventData.GCaMP7s.(dataType).whisk.Normdata(combWhiskLogical,:);
        ...
        [allWhiskFileIDs] = EventData.HbT.(dataType).whisk.fileIDs(combWhiskLogical,:);
        [allWhiskEventTimes] = EventData.HbT.(dataType).whisk.eventTime(combWhiskLogical,:);
        allWhiskDurations = EventData.HbT.(dataType).whisk.duration(combWhiskLogical,:);
        ...
        clear procWhiskGCaMP7sData procWhiskHbTData procWhiskCorticalMUAData procWhiskHippocampalMUAData procWhiskCorticalGamData procWhiskHippocampalGamData finalWhiskStartTimes finalWhiskEndTimes finalWhiskFiles
        dd = 1;
...
%                 whiskHbTarray = finalWhiskHbTData(cc,:);
                ...
                filtWhiskHbTarray = whiskHbTarray;%sgolayfilt(whiskHbTarray,3,17);
                filtWhiskGCaMP7sarray = whiskGCaMP7sarray;%sgolayfilt(whiskGCaMP7sarray,3,17);
        ...
        meanWhiskHbTData = mean(procWhiskHbTData,1)*100;
        stdWhiskHbTData = std(procWhiskHbTData,0,1)*100;
        ...
        % save results
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).HbT.HbT = meanWhiskHbTData;
        Results_Evoked.(animalID).Whisk.(dataType).(whiskCriteriaName).HbT.HbTStD = stdWhiskHbTData;
    ...
    for gg = 1:length(stimCriteriaNames)
        ...
        % pull data from EventData.mat structure
        allStimFilter = FilterEvents_IOS(EventData.HbT.(dataType).stim,StimCriteria);
        [allStimHbTData] = EventData.HbT.(dataType).stim.data(allStimFilter,:);
        [allStimGCaMP7sData] = EventData.GCaMP7s.(dataType).stim.data(allStimFilter,:); % EventData.GCaMP7s.(dataType).stim.Normdata(allStimFilter,:); 
        ...
        [allStimFileIDs] = EventData.HbT.(dataType).stim.fileIDs(allStimFilter,:);
        [allStimEventTimes] = EventData.HbT.(dataType).stim.eventTime(allStimFilter,:);
        ...
        clear procStimGCaMP7sData procStimHbTData procStimHbTData procStimCortMUAData procStimHipMUAData procStimCortGamData procStimHipGamData finalStimStartTimes finalStimEndTimes finalStimFiles
        ii = 1;
        for hh = 1:size(finalStimHbTData,1)
            ...
            if stimStartTime >= 0.5 && stimEndTime <= (trialDuration_sec - 0.5)
                ...
                filtStimHbTarray = stimHbTarray;%sgolayfilt(stimHbTarray,3,17);
                filtStimGCaMP7sarray = stimGCaMP7sarray;%sgolayfilt(stimGCaMP7sarray,3,17);
        ...
        meanStimHbTData = mean(procStimHbTData,1)*100;
        stdStimHbTData = std(procStimHbTData,0,1)*100;
        ...
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).HbT.HbT = meanStimHbTData;
        Results_Evoked.(animalID).Stim.(dataType).(solenoid).HbT.HbTStD = stdStimHbTData;
