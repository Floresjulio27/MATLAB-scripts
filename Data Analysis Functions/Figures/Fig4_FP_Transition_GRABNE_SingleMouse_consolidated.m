function [AnalysisResults] = Fig4_FP_Transition_GRABNE_SingleMouse_consolidated(rootFolder,saveFigs,delim,AnalysisResults,firstHrs)
...
ax1 = subplot(4,2,1);
p1 = plot(T1,mean([data.AWAKEtoNREM.mean_NE_Rhodamine;data.AWAKEtoNREM.mean_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',2);
hold on

plot(T1,mean([data.AWAKEtoNREM.mean_NE_Rhodamine;data.AWAKEtoNREM.mean_Ach_Rhodamine]) + mean([data.AWAKEtoNREM.std_NE_Rhodamine;data.AWAKEtoNREM.std_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',0.1);
plot(T1,mean([data.AWAKEtoNREM.mean_NE_Rhodamine;data.AWAKEtoNREM.mean_Ach_Rhodamine]) - mean([data.AWAKEtoNREM.std_NE_Rhodamine;data.AWAKEtoNREM.std_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',0.1);
ylabel({'\DeltaF/F (Z)'})
...
% NREM to Awake
ax2 = subplot(4,2,2);
p1 = plot(T1,mean([data.NREMtoAWAKE.mean_NE_Rhodamine;data.NREMtoAWAKE.mean_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',2);
hold on

plot(T1,mean([data.NREMtoAWAKE.mean_NE_Rhodamine;data.NREMtoAWAKE.mean_Ach_Rhodamine]) + mean([data.NREMtoAWAKE.std_NE_Rhodamine;data.NREMtoAWAKE.std_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',0.1);
plot(T1,mean([data.NREMtoAWAKE.mean_NE_Rhodamine;data.NREMtoAWAKE.mean_Ach_Rhodamine]) - mean([data.NREMtoAWAKE.std_NE_Rhodamine;data.NREMtoAWAKE.std_Ach_Rhodamine]),'-','color',[0.6350 0.0780 0.1840],'LineWidth',0.1);
ylabel({'\DeltaF/F (Z)'})
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure_D,[dirpath animalID 'Transition_Rhodamine_vs_GFP_singleMouse']);
    set(summaryFigure_D,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath animalID 'Transition_Rhodamine_vs_GFP_singleMouse'])
end
close
