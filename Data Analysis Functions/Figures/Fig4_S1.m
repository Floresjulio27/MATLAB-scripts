function [AnalysisResults] = Fig4_S1(rootFolder,saveFigs,delim,AnalysisResults)
...
for aa = 1:length(FPanimalIDs)
    ...
    for bb = 1:length(transitions)
        ...
        for cc = 1:length(uniqueFileDates)
            data.(animalID).(transition).(uniqueFileDates{cc,1}).LH_TRITC = [];
            data.(animalID).(transition).(uniqueFileDates{cc,1}).RH_TRITC = [];
        end
        for dd = 1:length(AnalysisResults.(animalID).Transitions.(transition).indFileDate)
            ...
            data.(animalID).(transition).(strDay).LH_TRITC = cat(1,data.(animalID).(transition).(strDay).LH_TRITC,AnalysisResults.(animalID).Transitions.(transition).LH_TRITC);%(dd,:));
            data.(animalID).(transition).(strDay).RH_TRITC = cat(1,data.(animalID).(transition).(strDay).RH_TRITC,AnalysisResults.(animalID).Transitions.(transition).RH_TRITC);%(dd,:));
        end
    end
end
...
for ee = 1:length(FPanimalIDs)
    ...
    for ff = 1:length(transitions)
        ...
        for gg = 1:length(uniqueFileDates)
            ...
            procData.(animalID).(transition).(strDay).TRITC = cat(1,data.(animalID).(transition).(strDay).LH_TRITC,data.(animalID).(transition).(strDay).RH_TRITC);
        end
    end
end
...
for ee = 1:length(FPanimalIDs)
    ...
    for ff = 1:length(transitions)
        ...
        for gg = 1:length(uniqueFileDates)
            strDay = uniqueFileDates{gg,1};
            if isempty(procData.(animalID).(transition).(strDay).TRITC) == false
                finData.(transition){gg,1}(ee,:) = mean(procData.(animalID).(transition).(strDay).TRITC,1);
            else
...
sgtitle('﻿Transitional changes in TRITC across days')
%% [4-S1a] Awake to NREM
ax1 = subplot(2,2,1);
for kk = 1:3
