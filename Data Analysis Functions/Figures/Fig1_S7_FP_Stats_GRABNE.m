function [AnalysisResults] = Fig1_S7_FP_Stats_GRABNE(rootFolder,saveFigs,delim,AnalysisResults,FP_animalIDs)
...
for aa = 1:length(FP_animalIDs)
    ...
    for bb = 1:length(behavFields)
        ...
        for cc = 1:length(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndAch)
            data.Rhodamine.(animalID).(behavField).meanAch(cc,1) = range(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndAch{cc,1});
        end
        for cc = 1:length(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndNE)
            data.Rhodamine.(animalID).(behavField).meanNE(cc,1) = range(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndNE{cc,1});
        end
    end
end
...
for aa = 1:length(FP_animalIDs)
    ...
    for bb = 1:length(behavFields)
        ...
        for cc = 1:length(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndAch)
            data.GFP.(animalID).(behavField).meanAch(cc,1) = range(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndAch{cc,1});
        end
        for cc = 1:length(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndNE)
            data.GFP.(animalID).(behavField).meanNE(cc,1) = range(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndNE{cc,1});
        end
    end
end
% put data into arrays and prep for stats
...
% Rhodamine_meanTable = table('Size',[size(tableSize,1),4],'VariableTypes',{'string','string','string','double'},'VariableNames',{'Mouse','Hemisphere','Behavior','Range'});
...
% Rhodamine_meanFitFormula = 'Range ~ 1 + Behavior + (1|Mouse) + (1|Mouse:Hemisphere)';
...
% GFP_meanTable = table('Size',[size(tableSize,1),4],'VariableTypes',{'string','string','string','double'},'VariableNames',{'Mouse','Hemisphere','Behavior','Range'});
...
% GFP_meanFitFormula = 'Range ~ 1 + Behavior + (1|Mouse) + (1|Mouse:Hemisphere)';
% GFP_meanStats = fitglme(GFP_meanTable,GFP_meanFitFormula)

%% Fig. 1-S4
summaryFigure = figure('Name','Fig1-S7 Stats');
sgtitle('Range hemodynamic changes')
%% Mean Rhodamine Ach
ax1 = subplot(2,2,1);
...
title({'Range Zscored \DeltaRhodamine Ach'})
ylabel('Range  \DeltaRhodamine Ach')
...
title({'Range Zscored \DeltaRhodamine NE'})
ylabel('Range  \DeltaRhodamine NE')
legend([s3,s1,s2,s4,s5],'Rest','Whisk','Stim','NREM','REM','Location','best')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
...
title({'Range Zscored \DeltaGFP Ach'})
ylabel('Range Zscored \DeltaGFP Ach')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
axis tight
...
title({'Range Zscored \DeltaGFP NE'})
ylabel('Range Zscored \DeltaGFP NE')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
axis tight
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure,[dirpath animalID '_Fig1-S7-Stats']);
    set(summaryFigure,'PaperPositionMode','auto');
    print('-painters','-dpdf','-bestfit',[dirpath animalID '_Fig1-S7-Stats'])
    close 
    %% Text diary
