function [AnalysisResults] = Fig1_S2_Stim_SST_Response_CI(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FPanimalIDs)
...
% % pq = plot(data.Ipsi.mean_timeVector,data.Ipsi.P_AChGFP_Mean - data.Contra.P_NEGFP_Mean,'-','color',[0 0.2 0.1],'LineWidth',2);
% % ax1.YAxis(1).Color = 'k';
% ax2.YAxis(2).Color = 'k';
xlabel('Peri-stimulus time (s)')
legend([p1,p2,p3,p4],'CBV LH', 'CBV RH', 'SST RH', 'SST LH')
axis square
set(gca,'box','off')
...
% CBV LH
ax2 = subplot(2,2,2);
p1= plot(data.Ipsi.mean_timeVector,data.Contra.P_AChCBV_Mean,'-','color',[0.8500 0.3250 0.0980],'LineWidth',2);
hold on
plot(data.Ipsi.mean_timeVector,data.Contra.P_AChCBV_Mean + data.Contra.P_AChCBV_yCI95,'-','color',[0.8500 0.3250 0.0980],'LineWidth',0.10)
title('Stimulus Evoked Response')
ylabel('\DeltaF/F')
% CBV RH
p2= plot(data.Contra.mean_timeVector,data.Ipsi.P_NECBV_Mean,'-','color',[0.6350 0.0780 0.1840],'LineWidth',2);
hold on
plot(data.Contra.mean_timeVector,data.Ipsi.P_NECBV_Mean + data.Ipsi.P_NECBV_yCI95,'-','color',[0.6350 0.0780 0.1840],'LineWidth',0.10)
title('Stimulus Evoked Response')
ylabel('\DeltaF/F')
% ax2.YLim = [-0.5 3];

% NE
p3 = plot(data.Contra.mean_timeVector,data.Ipsi.P_NEGFP_Mean,'-','color',[0.4660 0.6740 0.1880],'LineWidth',2);
hold on
plot(data.Contra.mean_timeVector,data.Ipsi.P_NEGFP_Mean + data.Ipsi.P_NEGFP_yCI95,'-','color',[0.4660 0.6740 0.1880],'LineWidth',0.10)
...
ylabel('\DeltaF/F')
% % pq = plot(data.Contra.mean_timeVector,data.Contra.P_AChGFP_Mean - data.Ipsi.P_NEGFP_Mean,'-','color',[0 0.2 0.1],'LineWidth',2);
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigureN,[dirpath ManipulationType '_' 'Stim-FiberSignals_response_CI_SST']); %animalID   
    set(summaryFigureN,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_'  'Stim-FiberSignals_response_CI_SST']) % animalID _NoSleep NoSleep_
    close 
end
...
% p4 = plot(data.Contra.mean_timeVector,data.Contra.P_AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
% 
% p5 =  plot(data.Contra.mean_timeVector,data.Ipsi.P_AChGFP_Mean-data.Contra.P_NEGFP_Mean,'k');
end
