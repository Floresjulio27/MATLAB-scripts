function [AnalysisResults] = Fig4_FP_Transition_SST_SingleMouse_consolidated_CI_PFC_FP(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FP_animalIDs)
...
% Blood Volume and SST Ca2+ activity in PFC
ax1 = subplot(2,2,1);
...
legend([p4,p2],'CBV PFC','SST PFC','Location','northeast','FontSize',5)
xlim([-30,30])
title('Awake to NREM')
set(gca,'box','off')
ax1.YAxis(1).Color = 'k';
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure_D,[dirpath 'Transition_CBV_vs_GFP_Sleep_CI_SST_PFC']);
    set(summaryFigure_D,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath 'Transition_CBV_vs_GFP_Sleep_CI_SST_PFC'])
end
close
