...
% clear all; 
% clc
function TDTFiberPhotometry_bilateralGFP(filepath,rawDataFilespath)
...
%             figure; plot(rescaleData_RH.F465);
            ...
            legend('465','560')
            title('Before Hemodynamic Correction'); xlabel('Time (min)'); legend({'465 ','560'}); xlim([0 figTime(end)]);
            ...
            plot(figTime,filtfilt(Params.sos_plot,Params.g_plot,CBVCorrection_RH.F560))
            legend('465','560')
            title('After Hemodynamic Correction'); xlabel('Time (min)'); legend({'465 ','560'}); xlim([0 figTime(end)]);
            ylabel('\Delta F rescaled');
            linkaxes(k,'x');
            ...
            title('Hemodynamic Corrected GFP Signals LH'); xlabel('Time (min)'); legend({'GFP ExpCorrect ','GFP RhodamineCorrect'}); xlim([0 figTime(end)]);
            ylabel('\Delta F');
            ...
            title('Hemodynamic Corrected GFP Signals RH'); xlabel('Time (min)'); legend({'GFP ExpCorrect ','GFP RhodamineCorrect'}); xlim([0 figTime(end)]);
            ylabel('\Delta F');
            linkaxes(h,'x');
            ...
            figure; plot(figTime,detrend(filtfilt(Params.sos_plot,Params.g_plot,CBVCorrection_RH.F465),'constant'))
            close all
...
%             [dF_RH, dFF0_RH,dFF0_z_RH] =  control405Correction(CBVCorrection_RH,predicted_RH,Params,'RH');
            [dF_LH, ~, dFF0_z_LH] =  control405Correction(CBVCorrection_LH,Params,'LH');
            [dF_RH, ~, dFF0_z_RH] =  control405Correction(CBVCorrection_RH,Params,'RH');
            ...
            title('Exp Corrected GFP Signals'); xlabel('Time (min)'); legend({'LH GFP','RH GFP'}); xlim([0 figTime(end)]);
            ylabel('\DeltaF');
            ...
            title('Final GFP Signals'); xlabel('Time (min)'); legend({'LH GFP','RH GFP'}); xlim([0 figTime(end)]);
            ylabel('\DeltaF/F (z)');
           ...
           % add some dummy values to match the index
            ...
            %% save the data
            % I don't have the LH recording for fibers
...
%             FiberData.LH.RhodamineCorrection = CBVCorrection_LH;
            FiberData.RH.RhodamineCorrection = CBVCorrection_RH;

%             FiberData.LH.dF = dF_LH;
            FiberData.RH.dF = dF_RH;
%              FiberData.LH.dFF0_z = dFF0_z_LH;
            FiberData.RH.dFF0_z = dFF0_z_RH;
