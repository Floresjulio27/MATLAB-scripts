% function FiberPhotometryAnalysis(filepath,rawDataFilespath,time_Trim_Start,time_Trim_End)
...
%%
    for trialNum = 1:1:length(Params.DataOnset)
        %% generate file save path for eACh trials
            ...
            Params.savepath =  ['FiberData_Hour' num2str(trialNum)];% [saveExt(1:strIdx-1) 'FiberData_Hour' num2str(trialNum)];
            ...
            if AnimalID == "NEAChM002" || AnimalID ==  "NEAChM003"
           ...
           % Match the data size
            ...
            clearvars -except -regexp Data$ path$ Params$ trialNum$ time_Trim_Start time_Trim_End fiberFile
            % saveExt = [rawDataFilespath{trialNum}];
            % strIdx = strfind(saveExt,'RawData');
            % savepathmat =  [saveExt(1:strIdx-1) 'FiberData.mat'];
            save(fiberFile,"FiberData",'-v7.3')
    end