...
%% main figure panels
[AnalysisResults] = Fig1_S4_FP_Stats_EEG(rootFolder,saveFigs,delim,AnalysisResults);
[AnalysisResults] = Fig1_S3_Whisk_EEG(rootFolder,saveFigs,delim,AnalysisResults);
[AnalysisResults] = Fig1_S2_Stim_EEG(rootFolder,saveFigs,delim,AnalysisResults);
[AnalysisResults] = Fig4_FP_Transition_EEG(rootFolder,saveFigs,delim,AnalysisResults);



% [AnalysisResults] = PlotCOherence_GRABNE(rootFolder,saveFigs,delim,AnalysisResults);
% [AnalysisResults] = PlotpowerSpectrum_GRABNE(rootFolder,saveFigs,delim,AnalysisResults);
% [AnalysisResults] = PlotCrossCorrelation_GRABNE(rootFolder,saveFigs,delim,AnalysisResults);
...
for aa = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,aa})) == false || isfield(AnalysisResults.(FP_animalIDs{1,aa}),'SleepProbability') == false || strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeAwakeProbability_EEG(FP_animalIDs{1,aa},saveFigs,rootFolder,AnalysisResults);
    end
...
for bb = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,bb})) == false || isfield(AnalysisResults.(FP_animalIDs{1,bb}),'BehaviorDistributions') == false || strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeBehavioralDistributions_EEG(FP_animalIDs{1,bb},rootFolder,AnalysisResults);
    end
...
for dd = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,dd})) == false || isfield(AnalysisResults.(FP_animalIDs{1,dd}),'Transitions') == false || strcmp(runFromStart,'y') == true
          [AnalysisResults] = AnalyzeTransitionalAverages_FP_Movements_EEG(FP_animalIDs{1,dd},saveFigs,rootFolder,AnalysisResults);
    end
...
for ff = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,ff})) == false || isfield(AnalysisResults.(FP_animalIDs{1,ff}),'MeanRhodamine') == false || strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeMeanRhodamine_FP_EEG(FP_animalIDs{1,ff},rootFolder,AnalysisResults);
    end
...
for ff = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,ff})) == false || isfield(AnalysisResults.(FP_animalIDs{1,ff}),'MeanGFP') == false || strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeMeanGFP_FP_EEG(FP_animalIDs{1,ff},rootFolder,AnalysisResults);
    end
...
for pp = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,pp})) == false || isfield(AnalysisResults.(FP_animalIDs{1,pp}),'Whisk') == false || isfield(AnalysisResults.(FP_animalIDs{1,pp}),'Stim') == false ||strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeEvokedResponses_EEG(FP_animalIDs{1,pp},rootFolder,AnalysisResults);
    end
...
%         [AnalysisResults] = AnalyzeCoherence_Pupil_GRABNE(FP_animalIDs{1,jj},rootFolder,AnalysisResults);
%     end
...
%         [AnalysisResults] = AnalyzeCrossCorrelation_GRABNE(FP_animalIDs{1,nn},rootFolder,AnalysisResults);
%     end
