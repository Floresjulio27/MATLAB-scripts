% function [figHandle,ax1,ax2,ax3,ax4,ax5,min_EEG_LH,max_EEG_LH] = Figures_Sleep_FP_EEG_CheckSleep(procDataFileID,saveFigs)
...
legend([s1,s2,s3,p5],'Awake','NREM','REM','EEG_LH')
set(gca,'TickLength',[0,0])
set(gca,'Xticklabel',[])
set(gca,'box','off')
axis tight
...
% Rhodamine and GFP
ax5 = subplot(7,1,5);
p10 = plot((1:length(filtRH_Rhodamine))/ProcData.notes.dsFs,filtRH_Rhodamine,'color','r','LineWidth',1);
ylabel('\Delta F/F')
legend('Rhodamine')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'Xticklabel',[])
...
ax7 = subplot(7,1,6);
p11 = plot((1:length(filtRH_GFP))/ProcData.notes.dsFs,filtRH_GFP,'color','g','LineWidth',1);
ylabel('\Delta F/F')
legend('GFP')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'Xticklabel',[])
