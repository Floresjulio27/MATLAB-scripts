function [AnalysisResults] = AnalyzeCrossCorrelation_DataDivision_SST(animalID,rootFolder,AnalysisResults,firstHrs)

% function parameters & data types
% dataTypes = {'NE_GFP','ACh_GFP'};
% pupilDataTypes = {'ACh_CBV','NE_CBV','ACh_GFP','NE_GFP'};
dataTypes = {'SST_PFC_GFP','SST_S1BF_GFP'};
pupilDataTypes = {'S1BF_CBV','PFC_CBV','SST_S1BF_GFP','SST_PFC_GFP'};
modelType = 'Manual';
...
for aa = 1:length(dataTypes)
    ...
    for bb = 1:length(pupilDataTypes)
      ...
      %I dont know where is the filter events function  % [restLogical] = FilterEvents_JNeurosci2022(RestData.GFP.P_NE,RestCriteria); %
        % [puffLogical] = FilterEvents_JNeurosci2022(RestData.GFP.P_NE,RestPuffCriteria);
        [restLogical] = FilterEvents_FP(RestData.GFP.P_NE,RestCriteria);
        [puffLogical] = FilterEvents_FP(RestData.GFP.P_NE,RestPuffCriteria);
