function [] = Fig_Sleep_Histograms2_SST_project(rootFolder,saveState,delim)
...
smoothedCounts    = smoothdata(totalNREM_allBins, 'gaussian', 3);

figure;
plot(1:length(smoothedCounts), smoothedCounts, '-o','LineWidth',1.5);
xticks(1:length(smoothedCounts));
xticklabels( Results_SleepData.Water.Bin_Labels );
xtickangle(45);
xlabel('Sleep‐Event Duration Bins');
ylabel('Smoothed Count (all days)');
title('Water → NREM (smoothed across bins, summed over days)');
...
title('Water Group - NREM Events (summed over all animals)');

% Legend: one entry per day
legend(days, 'Location', 'northeastoutside');

%% Plot NREM alcohol group across days 
alcoholIdx = strcmp(groups, 'Alcohol');  % logical index
alcoholIdx = find(alcoholIdx); % 1 should be for water

% Extract the matrix (nBins × nDays) for Water → NREM
allNREM_Alcohol = NREM_summary(:, :, alcoholIdx);  % size: [nBins × nDays]

figure;
bar(allNREM_Alcohol, 'grouped');
xticklabels(bin_labels);
xtickangle(45);
ylim([0 80])
xlabel('Sleep‐Event Duration Bins');
ylabel('Number of NREM Events');
title('Alcohol Group - NREM Events (summed over all animals)');
