function [Results_PowerSpectrum] = AnalyzePowerSpectrum_CBVandGFP_SST_project(animalID,group,setName,rootFolder,delim,Results_PowerSpectrum,days)
%________________________________________________________________________________________________________________________
% Written by Kevin L. Turner
% The Pennsylvania State University, Dept. of Biomedical Engineering
% https://github.com/KL-Turner
%
% Modified by: Md Shakhhawat Hossain 
% Adapted by: Julio Flores-Cuadra
%
%________________________________________________________________________________________________________________________
%
% Purpose: Analyze the spectral power of hemodynamic [CBV] and SST signal
%________________________________________________________________________________________________________________________
...
for aa = 1:length(dataTypes)
    ...
    if isempty(finalRestData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).S = rest_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).f = rest_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).sErr = rest_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Rest.(dataType).sErr = [];
    end
    ...
    if isempty(awakeData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).S = awake_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).f = awake_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).sErr = awake_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Awake.(dataType).sErr = [];
    end
    ...
    if isempty(asleepData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).S = asleep_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).f = asleep_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).sErr = asleep_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.Asleep.(dataType).sErr = [];
    end
    ...
    if isempty(allData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).S = all_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).f = all_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).sErr = all_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.All.(dataType).sErr = [];
    end
    ...
    if isempty(nremData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).S = nrem_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).f = nrem_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).sErr = nrem_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.NREM.(dataType).sErr = [];
    end
    ...
    if isempty(remData) == false
        ...
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).S = rem_S;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).f = rem_f;
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).sErr = rem_sErr;
    else
        % save results
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).S = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).f = [];
        Results_PowerSpectrum.(group).(days).(animalID).PowerSpectrum.REM.(dataType).sErr = [];
    end
end
%% save data
cd([rootFolder delim 'Results_SST_project'])
save('Results_PowerSpectrum_','Results_PowerSpectrum','-v7.3')
