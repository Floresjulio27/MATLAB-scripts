function [AnalysisResults] = PlotCrossCorrelation_SST_AAA(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,animalIDs)
...
%% cross correlation
    if firstHrs == "false"
        behavFields = {'NREM','Rest','Asleep','All','Alert'}; % 'Rest', 'REM',
...
SdataTypes ={'SST_PFC_GFP','SST_S1BF_GFP'}; %  {'AChNE'}; %
dataTypes = {'S1BF_CBV','PFC_CBV'}; %,'ACh_GFP','NE_GFP'
...
for bb = 1:length(dataTypes)
        ...
        Crosscorrelation_Fig = figure('Name','CrossCorrelation_Figure', 'Position', [100, 100, 1000, 1000]);
...
%             ylim([-0.35,0.15])
            ...
            % if strcmp(SdataType,dataType) == 1
                 % if firstHrs == "false"
            legend([ L4, L5, L6],'Asleep','All','Alert','Location','best');
     ...
     if strcmp(saveFigs,'y') == true
        ...
        savefig(Crosscorrelation_Fig,[dirpath ManipulationType '_Crosscorrelation_AwakeAsleepAlert_' (SdataType) '_' (dataType)]);
        set(Crosscorrelation_Fig,'PaperPositionMode','auto');
        print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_Crosscorrelation_AwakeAsleepAlert_' (SdataType) '_' (dataType)])
    end
    close
 end
