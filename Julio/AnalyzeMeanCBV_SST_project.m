function [Results_MeanCBV] = AnalyzeMeanCBV_SST_project(animalID,group,setName,rootFolder,delim,Results_MeanCBV,days)
...
restDataFileStruct = dir('*_RestData.mat'); %this is for skipping the days that do not have animals since I am still recording
if isempty(restDataFileStruct)
    return 
end 
...
% save results
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Rest.CBV.MeanACh = ACh_restCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Rest.CBV.MeanNE = NE_restCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Rest.CBV.IndACh = ACh_ProcRestData;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Rest.CBV.IndNE = NE_ProcRestData;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Rest.CBV.FileIDs = finalRestFileIDs;
...
% save results
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Whisk.CBV.MeanACh = ACh_whiskCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Whisk.CBV.MeanNE = NE_whiskCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Whisk.CBV.IndACh = ACh_whiskCBV;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Whisk.CBV.IndNE = NE_whiskCBV;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.Whisk.CBV.FileIDs = finalWhiskFileIDs;
...
if ~isempty(ACh_stimData) && ~isempty(ACh_stimEventTimes) && ... %There are some days that I did not stimulate. Skip those mice in those days
    ...
    for nn = 1:size(ACh_ProcStimData,1)
        ACh_stimCBVMean{nn,1} = mean(ACh_ProcStimData(nn,params.Offset*samplingRate:params.minTime.Stim*samplingRate),2);
        ACh_stimCBV{nn,1} = ACh_ProcStimData(nn,params.Offset*samplingRate:params.minTime.Stim*samplingRate);
    end
    % take mean [CBV] 1-2 seconds after stimulation (right hem)
    for nn = 1:size(NE_ProcStimData,1)
        NE_stimCBVMean{nn,1} = mean(NE_ProcStimData(nn,params.Offset*samplingRate:params.minTime.Stim*samplingRate),2);
        NE_stimCBV{nn,1} = NE_ProcStimData(nn,params.Offset*samplingRate:params.minTime.Stim*samplingRate);
    end
    % save results
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.MeanACh = cell2mat(ACh_stimCBVMean);
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.MeanNE = cell2mat(NE_stimCBVMean);
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.IndACh = ACh_stimCBV;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.IndNE = NE_stimCBV;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.ACh_FileIDs = ACh_finalStimFileIDs;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.Stim.CBV.NE_FileIDs = NE_finalStimFileIDs;
end
...
% save results
Results_MeanCBV.(group).(days).(animalID).MeanCBV.NREM.CBV.MeanACh = AChremCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.NREM.CBV.MeanNE = NEremCBVMean;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.NREM.CBV.IndACh = AChremData;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.NREM.CBV.IndNE = NEremData;
Results_MeanCBV.(group).(days).(animalID).MeanCBV.NREM.CBV.FileIDs = nremFileIDs;
...
if isfield(SleepData.(modelType),'REM')
    ...
    % save results
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.REM.CBV.MeanACh = ACh_remCBVMean;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.REM.CBV.MeanNE = NE_remCBVMean;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.REM.CBV.IndACh = ACh_remData;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.REM.CBV.IndNE = NE_remData;
    Results_MeanCBV.(group).(days).(animalID).MeanCBV.REM.CBV.FileIDs = remFileIDs; 
end

%% save data
    cd([rootFolder delim 'Results_SST_project'])
    save('Results_MeanCBV.mat','Results_MeanCBV')
