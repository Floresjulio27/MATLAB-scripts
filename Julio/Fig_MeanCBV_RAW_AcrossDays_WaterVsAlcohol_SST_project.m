function [] = Fig_MeanCBV_RAW_AcrossDays_WaterVsAlcohol_SST_project(rootFolder,saveState,delim)
...
cd(path)
resultsStruct_CBV = 'Results_MeanCBV';
load(resultsStruct_CBV)
...
for ee = 1:length(groups)
    ...
    for hemi = 1:length(hemisphere)
        ...
        for xx = 1:length(days)
            ...
            if ~isfield(Results_MeanCBV.(currentGroup), currentDay)
                continue
            end

            FPanimalIDs = fieldnames(Results_MeanCBV.(currentGroup).(currentDay));
            ...
            for aa = 1:length(FPanimalIDs)
                ...
                for behavi = 1:length(behavior)
                    ...
                    if ~isfield(Results_MeanCBV.(currentGroup).(currentDay).(animalID).MeanCBV, currentBehavior)
                        continue
                    end

                    %Calculate the mean of all events within one mouse
                    rawEvents = Results_MeanCBV.(currentGroup).(currentDay).(animalID).MeanCBV.(currentBehavior).CBV.(currentHemisphere);
                    mouseMean = mean(rawEvents,1); %k<AGROw>

                    %store the mean for plotting later
                    data.(currentGroup).(currentDay).(animalID).MeanCBV.(currentHemisphere).([currentBehavior '_Events']) =  rawEvents;
                    data.(currentGroup).(currentDay).(animalID).MeanCBV.(currentHemisphere).([currentBehavior '_Mean'])  =  mouseMean;
                end
            end
        end
    end
        ...
        for ee = 1:length(groups)
            ...
            for xx = 1:length(days)
                ...
                for aa = 1:length(FPanimalIDs)
                    ...
                    if isfield(data.(currentGroup).(currentDay).(animalID).MeanCBV.(currentHemisphere), [currentBehavior '_Mean'])
                        currentMean = data.(currentGroup).(currentDay).(animalID).MeanCBV.(currentHemisphere).([currentBehavior '_Mean']);
...
title('Rest - PFC'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([-0.4 0.1])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('Whisk - PFC'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([0 1.3])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('Stim - PFC'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([0 2.5])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('NREM - PFC'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)'); 
xlim([0.5 6.5])
ylim([-0.2 0.6])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('REM - PFC'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)'); 
xlim([0.5 6.5])
ylim([0 4.5])


sgtitle('PFC CBV - Water vs Alcohol - All Behaviors');
legend({'Water','Alcohol'});
%% Figure 2: S1BF data 
currentHemisphere = 'MeanACh';
fig2 = figure('Name','S1BF CBV');
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('Rest - S1BF'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([-0.2 0.2])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('Whisk - S1BF'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([-0.7 0.7])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('Stim - S1BF'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)'); 
xlim([0.5 6.5])
ylim([-1.0 1.5])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('NREM - S1BF'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([0 1.5])
...
errorbar(1:6, ...
    ...
    '-o','Color',alcoholColor,'LineWidth',2);
title('REM - S1BF'); xticks(1:6); xticklabels(days); ylabel('Mean CBV (%)');
xlim([0.5 6.5])
ylim([-0.5 3.0])

sgtitle('S1BF CBV - Water vs Alcohol - All Behaviors');
legend({'Water','Alcohol'});


%% Figure 3: Bar graph with individual data points
currentHemisphere = 'MeanNE';
...
fig3 = figure('Name','PFC CBV - All Behaviors - Group + Individual');
...
% SCATTER PLOT: WATER MICE
    for d = 1:6
        ...
        for aa = 1:length(mouseIDs)
            if isfield(data.Water.(days{d}).(mouseIDs{aa}).MeanCBV.(currentHemisphere), [currentBehavior '_Mean'])
                values(end+1) = data.Water.(days{d}).(mouseIDs{aa}).MeanCBV.(currentHemisphere).([currentBehavior '_Mean']);
            end
        end
...
% SCATTER PLOT: ALCOHOL MICE 
    for d = 1:6
        ...
        values = [];
        for aa = 1:length(mouseIDs)
            if isfield(data.Alcohol.(days{d}).(mouseIDs{aa}).MeanCBV.(currentHemisphere), [currentBehavior '_Mean'])
                values(end+1) = data.Alcohol.(days{d}).(mouseIDs{aa}).MeanCBV.(currentHemisphere).([currentBehavior '_Mean']);
            end
        end
...
sgtitle('PFC CBV- All Behaviors - Group + Individual');
...
%% Save figure
if saveState == true
    dirpath = [rootFolder delim 'MATLAB Figures Water vs Alcohol' delim 'MeanCBV'];
    if ~exist(dirpath,'dir')
        mkdir(dirpath);
    end
    savefig(fig1, fullfile(dirpath, 'Mean CBV PFC - All Behaviors'));
    savefig(fig2, fullfile(dirpath, 'Mean CBV S1BF - All Behaviors'));
    savefig(fig3, fullfile(dirpath, 'Individual data points mean CBV PFC - All Behaviors'));
    savefig(fig4, fullfile(dirpath, 'Individual data points mean CBV S1BF - All Behaviors'));
end