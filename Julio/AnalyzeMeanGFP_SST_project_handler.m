function [] = AnalyzeMeanGFP_SST_project_handler(rootFolder,delim,runFromStart)
%----------------------------------------------------------------------------------------------------------
...
if runFromStart == true
    Results_MeanGFP.Water   = struct();
    Results_MeanGFP.Alcohol = struct();
else
    cd(fullfile(rootFolder,'Results_SST_project'));
    if exist('Results_MeanGFP.mat','file')
        load('Results_MeanGFP.mat','Results_MeanGFP')
    else
        Results_MeanGFP.Water   = struct();
        Results_MeanGFP.Alcohol = struct();
    end
...
% 4) Run analysis
cc = 1;
multiWaitbar('Analyzing MeanGFP', 0, 'Color','P');
for ia = 1:length(groups)
  for id = 1:length(days)
    ...
    for ib = 1:length(animalIDs)
      ...
      if ~isfield(Results_MeanGFP.(grp), dayName) || ~isfield(Results_MeanGFP.(grp).(dayName), aID)
          Results_MeanGFP = AnalyzeMeanGFP_SST_project(aID, grp, setName, rootFolder, delim, Results_MeanGFP, dayName);

        % ensure I am back in Data folder
        cd(dataPath);
      end

      multiWaitbar('Analyzing MeanGFP', cc/waitBarLength);
      pause(0.1);
      cc = cc + 1;
    end
  end
