function [Results_Transitions] = AnalyzeArousalTransitions_SST_project(animalID,group,setName,rootFolder,delim,Results_Transitions,days)
...
if isempty(modelDataFileStruct)
    return
end
...
for d = 1:length(transitions)
    ...
    for mvn =  1:length(movecond)
        ...
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).Idx = data.(transition).(movementname);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).whisk = nanmean(data.(transition).whisk(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).force = nanmean(data.(transition).force(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).EMG = nanmean(data.(transition).EMG(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).T = data.(transition).T_short;
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).F = data.(transition).F;
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).indFileDate = data.(transition).fileDate(data.(transition).(movementname));
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).fileDates = fileDates;
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).LH_Cort = nanmean(data.(transition).LH_Cort(:,:,data.(transition).(movementname)),3);

        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).ACh_CBV = nanmean(data.(transition).ACh_CBV(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).NE_CBV = nanmean(data.(transition).NE_CBV(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).GRAB_ACh = nanmean(data.(transition).GRAB_ACh(data.(transition).(movementname),:),1);
        Results_Transitions.(group).(days).(animalID).Transitions.(transition).(movementname).GRAB_NE = nanmean(data.(transition).GRAB_NE(data.(transition).(movementname),:),1);
    end
    %% this is the whole results
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).whisk = mean(data.(transition).whisk,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).force = mean(data.(transition).force,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).whisk_std = std(data.(transition).whisk,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).force_std = std(data.(transition).force,1);
    
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).EMG = mean(data.(transition).EMG,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).EMG_std = std(data.(transition).EMG,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).T = data.(transition).T_short;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).F = data.(transition).F;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).indFileDate = data.(transition).fileDate;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).fileDates = fileDates;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).LH_Cort = mean(data.(transition).LH_Cort,3);

    Results_Transitions.(group).(days).(animalID).Transitions.(transition).ACh_CBV = mean(data.(transition).ACh_CBV,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).NE_CBV = mean(data.(transition).NE_CBV,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_ACh = mean(data.(transition).GRAB_ACh,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_NE = mean(data.(transition).GRAB_NE,1);

    Results_Transitions.(group).(days).(animalID).Transitions.(transition).ACh_CBVRaw = data.(transition).ACh_CBV;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).NE_CBVRaw = data.(transition).NE_CBV;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_AChRaw = data.(transition).GRAB_ACh;
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_NERaw = data.(transition).GRAB_NE;

    Results_Transitions.(group).(days).(animalID).Transitions.(transition).ACh_CBV_std = std(data.(transition).ACh_CBV,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).NE_CBV_std = std(data.(transition).NE_CBV,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_ACh_std = std(data.(transition).GRAB_ACh,1);
    Results_Transitions.(group).(days).(animalID).Transitions.(transition).GRAB_NE_std = std(data.(transition).GRAB_NE,1);
end
 % save data
