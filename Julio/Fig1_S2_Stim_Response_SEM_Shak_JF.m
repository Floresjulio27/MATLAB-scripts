function [AnalysisResults] = Fig1_S2_Stim_Response_SEM_Shak_JF(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FPanimalIDs)
...
p3 = plot(data.Ipsi.mean_timeVector,data.Ipsi.P_AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
hold on
AChn(1,:) =  data.Ipsi.P_AChGFP_Mean + data.Ipsi.P_AChGFP_SEM; %
AChn(2,:) =  data.Ipsi.P_AChGFP_Mean - data.Ipsi.P_AChGFP_SEM; %
patch([data.Ipsi.mean_timeVector' fliplr(data.Ipsi.mean_timeVector')], [AChn(1,:) fliplr(AChn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);

ylabel('\DeltaF/F (%)')

x = [0 1 1 0];
y = [-0.5 -0.5 2.75 2.75];
patch(x,y,'b','FaceColor','b','EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.1);
...
% ACh
p3 = plot(data.Contra.mean_timeVector,data.Contra.P_AChGFP_Mean,'-','color',[0 0.4470 0.7410],'LineWidth',2);
hold on
AChn(1,:) =  data.Contra.P_AChGFP_Mean + data.Contra.P_AChGFP_SEM;
AChn(2,:) =  data.Contra.P_AChGFP_Mean - data.Contra.P_AChGFP_SEM;

patch([data.Contra.mean_timeVector' fliplr(data.Contra.mean_timeVector')], [AChn(1,:) fliplr(AChn(2,:))],[0 0.4470 0.7410],'FaceColor',[0 0.4470 0.7410],'EdgeColor','none','LineWidth',0.1,'FaceAlpha',0.2);
...
axis square
    ...
    if strcmp(saveFigs,'y') == true
        ...
        savefig(summaryFigureN,[dirpath ManipulationType '_' 'Stim_Response_SST_SEM']);  
        set(summaryFigureN,'PaperPositionMode','auto');
        print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_'  'Stim_Response_SST_SEM']) 
        close 
    end
end
