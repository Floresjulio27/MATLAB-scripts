...
%% Load data
baseline = load('Fig1_S2_Baseline1_Data.mat'); data_baseline = baseline.data;
did4 = load('Fig1_S2_DID4_Data.mat'); data_did4 = did4.data;

summaryFigureOverlay = figure;
sgtitle('Stimulus-evoked responses: Baseline vs DID4')
%% Plotting 

%set colors 
gfp_base_color = [0 0 0];
gfp_did4_color = [0.55, 0.45, 0.70];
cbv_base_color = [0.30, 0.30, 0.30];
cbv_did4_color = [0.70, 0.40, 0.10];

% Contra stim PFC fiber GFP (Right hemisphere, left whisker stim)
ax1 = subplot(2,2,1);

% GFP Baseline
x = data_baseline.Contra.mean_timeVector(:)';
y = data_baseline.Contra.P_NEGFP_Mean(:)';
e = data_baseline.Contra.P_NEGFP_SEM(:)';
fill([x fliplr(x)], [y+e fliplr(y-e)], gfp_base_color,'FaceAlpha', 0.2, 'EdgeColor', 'none'); 
hold on;
p1 = plot(x, y, 'Color', gfp_base_color, 'LineWidth', 2);

% GFP DID4 
x2 = data_did4.Contra.mean_timeVector(:)';
y2 = data_did4.Contra.P_NEGFP_Mean(:)';
e2 = data_did4.Contra.P_NEGFP_SEM(:)';
fill([x2 fliplr(x2)], [y2+e2 fliplr(y2-e2)], gfp_did4_color,'FaceAlpha', 0.2, 'EdgeColor', 'none');
p2 = plot(x2, y2, 'Color', gfp_did4_color, 'LineWidth', 2);

title('Contra stim PFC fiber GFP')
ylabel('\DeltaF/F SST Ca2+ activity (%)')
xlabel('Time (s)')
legend([p1 p2],'Baseline', 'DID4')
axis square; 
axis tight;
xlim([-5 15]);
ylim([-0.4 1.5]);
...
%CBV baseline
x3 = data_baseline.Contra.mean_timeVector(:)';
y3 = data_baseline.Contra.P_NECBV_Mean(:)';
e3 = data_baseline.Contra.P_NECBV_SEM(:)';
...
%CBV DID4
x4 = data_did4.Contra.mean_timeVector(:)';
y4 = data_did4.Contra.P_NECBV_Mean(:)';
e4 = data_did4.Contra.P_NECBV_SEM(:)';
fill([x4 fliplr(x4)], [y4+e4 fliplr(y4-e4)], cbv_did4_color,'FaceAlpha', 0.2, 'EdgeColor', 'none'); 
hold on;
p4 = plot(x4, y4, 'Color', cbv_did4_color, 'LineWidth', 2);

title('Contra stim PFC fiber CBV')
ylabel('\DeltaF/F CBV (%)')
xlabel('Time (s)')
legend([p3 p4],'Baseline', 'DID4')
axis square; 
axis tight;
xlim([-5 15]);
ylim([-0.5 4.0])
