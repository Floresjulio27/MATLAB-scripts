...
[AnalysisResults] = Fig1_S3_EEG(rootFolder,saveFigs,delim,AnalysisResults);
[AnalysisResults] = Fig1_S2_contra_EEG(rootFolder,saveFigs,delim,AnalysisResults);
...
% IOS animal IDs
FP_animalIDs = {'SHF025','SHF026','SHF027'};
saveFigs = 'y';
...
for pp = 1:length(FP_animalIDs)
    if isfield(AnalysisResults,(FP_animalIDs{1,pp})) == false || isfield(AnalysisResults.(FP_animalIDs{1,pp}),'EvokedAvgs') == false || strcmp(runFromStart,'y') == true
        [AnalysisResults] = AnalyzeEvokedResponses_EEG(FP_animalIDs{1,pp},saveFigs,rootFolder,AnalysisResults);
    end
