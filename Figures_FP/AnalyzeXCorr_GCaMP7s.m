function [AnalysisResults] = AnalyzeXCorr_GCaMP7s(animalID,rootFolder,AnalysisResults)
...
%________________________________________________________________________________________________________________________
%
%   Purpose: Analyze the cross-correlation between neural activity and GCaMP7s
%________________________________________________________________________________________________________________________
...
% lowpass filter
samplingRate = RestData.GCaMP7s.LH.CBVCamSamplingRate;
...
for aa = 1:length(dataTypes)
    ...
    trialDuration_sec = RestData.GCaMP7s.LH.trialDuration_sec;
    sleepBinWidth = 5;   % sec
    oneSecSpecFs = 30;   % Hz
    %% cross-correlation analysis for resting data
    % pull data from RestData.mat structure
    [restLogical] = FilterEvents_IOS(RestData.GCaMP7s.(dataType),RestCriteria);
    [puffLogical] = FilterEvents_IOS(RestData.GCaMP7s.(dataType),RestPuffCriteria);
    combRestLogical = logical(restLogical.*puffLogical);
    restFileIDs = RestData.GCaMP7s.(dataType).fileIDs(combRestLogical,:);
    restDurations = RestData.GCaMP7s.(dataType).durations(combRestLogical,:);
    restEventTimes = RestData.GCaMP7s.(dataType).eventTimes(combRestLogical,:);
    
    restingGCaMP7sData = RestData.GCaMP7s.(dataType).data(combRestLogical,:);
    restingMUAData = RestData.(neuralDataType).muaPower.NormData(combRestLogical,:);
    % keep only the data that occurs within the manually-approved awake regions
    [restFinalRestGCaMP7sData,restFinalFileIDs,restFinalDurations,restFinalEventTimes] = RemoveInvalidData_IOS(restingGCaMP7sData,restFileIDs,restDurations,restEventTimes,ManualDecisions);
    ...
    for bb = 1:length(restFinalFileIDs)
        ...
        if restStartTime >= 0.5 && (restStartTime + restDuration) <= (trialDuration_sec - 0.5)
            % remove the number of samples due to rounding up to start and rounding down to end. This is done to keep the GCaMP7s/MUA vectores aligned positionally with the upcoming
            ...
            % load in GCaMP7s from rest period
            restGCaMP7s = restFinalRestGCaMP7sData{bb,1};
            restMUA = restFinalRestMUAData{bb,1};
            % remove leading/lag samples due to rounding to nearest 0.1 up/0.1 down
            restSnipGCaMP7s = restGCaMP7s(1 + leadSamples:end - lagSamples);
            restSnipMUA = restMUA(1 + leadSamples:end - lagSamples);
            restFiltGCaMP7s = filtfilt(sos,g,detrend(restSnipGCaMP7s,'constant'));
            ...
            if length(restFiltGCaMP7s) < params.minTime.Rest*samplingRate
                restChunkSampleDiff = params.minTime.Rest*samplingRate - length(restFiltGCaMP7s);
                restPadGCaMP7s = (ones(1,restChunkSampleDiff))*restFiltGCaMP7s(end);
                restPadMUA = (ones(1,restChunkSampleDiff))*restFiltMUA(end);
                restShortGCaMP7s = horzcat(restFiltGCaMP7s,restPadGCaMP7s);
                restShortMUA = horzcat(restFiltMUA,restPadMUA);
            else
                restShortGCaMP7s = restFiltGCaMP7s(1:params.minTime.Rest*samplingRate);
                restShortMUA = restFiltMUA(1:params.minTime.Rest*samplingRate);
            end
            restProcData.GCaMP7s{cc,1} = restShortGCaMP7s;
        ...
        restGCaMP7svLFPzhold = [];
        restLagTime = 5;   % seconds
        restFrequency = oneSecSpecFs;   % Hz
        restMaxLag = restLagTime*restFrequency;
        restGCaMP7svLFPxcVals = ones(length(rest_F),2*restMaxLag + 1);
        % run cross-correlation analysis - average through time
        for dd = 1:length(restProcData.GCaMP7s)
            for ee = 1:size(restProcData.S{dd, 1}, 1)
                restGCaMP7sarray = restProcData.GCaMP7s{dd,1};
                restMUAarray = restProcData.MUA{dd,1};
                restNeuralArray = restProcData.S{dd,1}(ee,:);
                [restGCaMP7svLFPxcVals(ee,:),restLFP_lags] = xcorr(restGCaMP7sarray,restNeuralArray,restMaxLag,'coeff');
            end
            [restGCaMP7svMUAxcVals(dd,:),restMUA_lags] = xcorr(restGCaMP7sarray,restMUAarray,restMaxLag,'coeff'); %#ok<*AGROW>
            restGCaMP7svLFPzhold = cat(3,restGCaMP7svLFPzhold,restGCaMP7svLFPxcVals);
        end
        restMeanGCaMP7svLFPxcVals = mean(restGCaMP7svLFPzhold,3);
        restMeanGCaMP7svMUAxcVals = mean(restGCaMP7svMUAxcVals,1);
        restStdGCaMP7svMUAxcVals = std(restGCaMP7svMUAxcVals,0,1);
    end
    % save results
    ...
    AnalysisResults.(animalID).XCorr.Rest.(dataType).GCaMP7svLFPxcVals = restMeanGCaMP7svLFPxcVals;
    AnalysisResults.(animalID).XCorr.Rest.(dataType).GCaMP7svMUAxcVals = restMeanGCaMP7svMUAxcVals;
    AnalysisResults.(animalID).XCorr.Rest.(dataType).GCaMP7svMUAxcVals_std = restStdGCaMP7svMUAxcVals;
    %% cross-correlation analysis for NREM
    NREM_sleepTime = params.minTime.NREM;   % seconds
    [NREM_finalGCaMP7s,NREM_allSleepFileIDs,NREM_finalBinTimes] = RemoveStimSleepData_IOS(animalID,SleepData.(modelType).NREM.data.GCaMP7s.(dataType),SleepData.(modelType).NREM.FileIDs,SleepData.(modelType).NREM.BinTimes);
    ...
    % adjust[GCaMP7s] and MUA events to match the edits made to the length of each spectrogram
    mm = 1;
    for ll = 1:length(NREM_dtSleepNeuralVals)
        if isempty(NREM_dtSleepNeuralVals) == false
            ...
            if strcmp(editIndex{ll,1},'none') == true
                NREM_GCaMP7sVals = NREM_finalGCaMP7s{ll,1}(1:NREM_sleepTime*samplingRate);
                NREM_MUAVals = NREM_finalMUA{ll,1}(1:NREM_sleepTime*samplingRate);
                NREM_finalGCaMP7sVals{mm,1} = filtfilt(sos,g,detrend(NREM_GCaMP7sVals,'constant'));
                NREM_finalMUAVals{mm,1} = filtfilt(sos,g,detrend(NREM_MUAVals,'constant'));
                mm = mm + 1;
            elseif strcmp(editIndex{ll,1},'leading') == true
                NREM_GCaMP7sVals = NREM_finalGCaMP7s{ll,1}((samplingRate*sleepBinWidth) + 1:(NREM_sleepTime*samplingRate + samplingRate*sleepBinWidth));
            ...
            elseif strcmp(editIndex{ll,1},'delete') == true
                % remove GCaMP7s/MUA from final file
            end
        end
    end
    % run cross-correlation analysis - average through time
    NREM_F = SpecData.(neuralDataType).F;
    NREM_GCaMP7svLFPzHold = [];
    NREM_lagTime = 5;   % Seconds
    NREM_frequency = oneSecSpecFs;   % Hz
    NREM_maxLag = NREM_lagTime*NREM_frequency;
    NREM_GCaMP7svLFPxcVals = ones(size(NREM_indSleepNeuralVals,2),2*NREM_maxLag + 1);
    for nn = 1:length(NREM_finalSleepNeuralVals)
        for oo = 1:size(NREM_finalSleepNeuralVals{nn,1},1)
            NREM_GCaMP7s_array = NREM_finalGCaMP7sVals{nn,1};
            NREM_MUA_array = NREM_finalMUAVals{nn,1};
            NREM_Neural_array = NREM_finalSleepNeuralVals{nn,1}(oo,:);
            [NREM_GCaMP7svLFPxcVals(oo,:),NREM_LFP_lags] = xcorr(NREM_GCaMP7s_array,NREM_Neural_array,NREM_maxLag,'coeff');
        end
        [NREM_GCaMP7svMUAxcVals(nn,:),NREM_MUA_lags] = xcorr(NREM_GCaMP7s_array,NREM_MUA_array,NREM_maxLag,'coeff');
        NREM_GCaMP7svLFPzHold = cat(3,NREM_GCaMP7svLFPzHold,NREM_GCaMP7svLFPxcVals);
    end
    NREM_meanGCaMP7svLFPxcVals = mean(NREM_GCaMP7svLFPzHold,3);
    NREM_meanGCaMP7svMUAxcVals = mean(NREM_GCaMP7svMUAxcVals,1);
    NREM_stdGCaMP7svMUAxcVals = std(NREM_GCaMP7svMUAxcVals,0,1);
    % save results
    ...
    AnalysisResults.(animalID).XCorr.NREM.(dataType).GCaMP7svLFPxcVals = NREM_meanGCaMP7svLFPxcVals;
    AnalysisResults.(animalID).XCorr.NREM.(dataType).GCaMP7svMUAxcVals = NREM_meanGCaMP7svMUAxcVals;
    AnalysisResults.(animalID).XCorr.NREM.(dataType).GCaMP7svMUAxcVals_std = NREM_stdGCaMP7svMUAxcVals;
    %% cross-correlation analysis for REM
%{
    REM_sleepTime = params.minTime.REM;   % seconds
    [REM_finalGCaMP7s,REM_allSleepFileIDs,REM_finalBinTimes] = RemoveStimSleepData_IOS(animalID,SleepData.(modelType).REM.data.GCaMP7s.(dataType),SleepData.(modelType).REM.FileIDs,SleepData.(modelType).REM.BinTimes);
    ...
    for ww = 1:length(REM_dtSleepNeuralVals)
        if isempty(REM_dtSleepNeuralVals) == false
            ...
            if strcmp(editIndex{ww,1},'none') == true
                REM_GCaMP7sVals = REM_finalGCaMP7s{ww,1}(1:REM_sleepTime*samplingRate);
                REM_MUAVals = REM_finalMUA{ww,1}(1:REM_sleepTime*samplingRate);
                REM_finalGCaMP7sVals{xx,1} = filtfilt(sos,g,detrend(REM_GCaMP7sVals,'constant'));
                REM_finalMUAVals{xx,1} = filtfilt(sos,g,detrend(REM_MUAVals,'constant'));
                xx = xx + 1;
            elseif strcmp(editIndex{ww,1},'leading') == true
                REM_GCaMP7sVals = REM_finalGCaMP7s{ww,1}((samplingRate*sleepBinWidth) + 1:(REM_sleepTime*samplingRate + samplingRate*sleepBinWidth));
    ...
    REM_GCaMP7svLFPzHold = [];
    REM_lagTime = 5;   % Seconds
    REM_frequency = oneSecSpecFs;   % Hz
    REM_maxLag = REM_lagTime*REM_frequency;
    REM_GCaMP7svLFPxcVals = ones(size(REM_indSleepNeuralVals,2),2*REM_maxLag + 1);
    for yy = 1:length(REM_finalSleepNeuralVals)
        for zz = 1:size(REM_finalSleepNeuralVals{yy,1},1)
            REM_GCaMP7s_array = REM_finalGCaMP7sVals{yy,1};
            REM_MUA_array = REM_finalMUAVals{yy,1};
            REM_Neural_array = REM_finalSleepNeuralVals{yy,1}(zz,:);
            [REM_GCaMP7svLFPxcVals(zz,:),REM_LFP_lags] = xcorr(REM_GCaMP7s_array,REM_Neural_array,REM_maxLag,'coeff');
        end
        [REM_GCaMP7svMUAxcVals(yy,:),REM_MUA_lags] = xcorr(REM_GCaMP7s_array,REM_MUA_array,REM_maxLag,'coeff');
        REM_GCaMP7svLFPzHold = cat(3,REM_GCaMP7svLFPzHold,REM_GCaMP7svLFPxcVals);
    end
    REM_meanGCaMP7svLFPxcVals = mean(REM_GCaMP7svLFPzHold,3);
    REM_meanGCaMP7svMUAxcVals = mean(REM_GCaMP7svMUAxcVals,1);
    REM_stdGCaMP7svMUAxcVals = std(REM_GCaMP7svMUAxcVals,0,1);
    % save results
    ...
    AnalysisResults.(animalID).XCorr.REM.(dataType).GCaMP7svLFPxcVals = REM_meanGCaMP7svLFPxcVals;
    AnalysisResults.(animalID).XCorr.REM.(dataType).GCaMP7svMUAxcVals = REM_meanGCaMP7svMUAxcVals;
    AnalysisResults.(animalID).XCorr.REM.(dataType).GCaMP7svMUAxcVals_std = REM_stdGCaMP7svMUAxcVals;
    %}
end
% save data
cd(rootFolder)
