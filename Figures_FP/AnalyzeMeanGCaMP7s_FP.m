function [AnalysisResults] = AnalyzeMeanGCaMP7s_FP(animalID,rootFolder,AnalysisResults)
...
% lowpass filter
samplingRate = RestData.GCaMP7s.LH.RhodamineCamSamplingRate;
...
for nn = 1:size(LH_ProcStimData,1)
    LH_stimGCaMP7sMean{nn,1} = mean(LH_ProcStimData(nn,(params.Offset + 2)*samplingRate:params.minTime.Stim*samplingRate),2);
    LH_stimGCaMP7s{nn,1} = LH_ProcStimData(nn,(params.Offset + 2)*samplingRate:params.minTime.Stim*samplingRate);
end
% take mean [GCaMP7s] 1-2 seconds after stimulation (right hem)
for nn = 1:size(RH_ProcStimData,1)
    RH_stimGCaMP7sMean{nn,1} = mean(RH_ProcStimData(nn,(params.Offset + 2)*samplingRate:params.minTime.Stim*samplingRate),2);
    RH_stimGCaMP7s{nn,1} = RH_ProcStimData(nn,(params.Offset + 2)*samplingRate:params.minTime.Stim*samplingRate);
end
% save results
