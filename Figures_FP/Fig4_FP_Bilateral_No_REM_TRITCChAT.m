function [AnalysisResults] = Fig4_FP_Bilateral_No_REM_RhodamineChAT(rootFolder,saveFigs,delim,AnalysisResults)
...
plot(T1,data.AWAKEtoNREM.meanRhodamine + data.AWAKEtoNREM.stdRhodamine,'-','color',colors('dark candy apple red'),'LineWidth',0.1);
plot(T1,data.AWAKEtoNREM.meanRhodamine - data.AWAKEtoNREM.stdRhodamine,'-','color',colors('dark candy apple red'),'LineWidth',0.1);
...
hold on
plot(T1,data.NREMtoAWAKE.meanRhodamine + data.NREMtoAWAKE.stdRhodamine,'-','color',colors('dark candy apple red'),'LineWidth',0.1);
plot(T1,data.NREMtoAWAKE.meanRhodamine - data.NREMtoAWAKE.stdRhodamine,'-','color',colors('dark candy apple red'),'LineWidth',0.1);
ylabel('\DeltaRhodamine')
xlim([-30,30])
% ylim([-5,50])
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure_A,[dirpath 'Fig4_A_RhodamineChAT']);
    set(summaryFigure_A,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath 'Fig4_A_RhodamineChAT'])
end
close
