function [SleepData] = CreateSleepData_FP(startingDirectory,trainingDirectory,baselineDirectory,NREMsleepTime,REMsleepTime,modelName,SleepData)
...
for a = 1:size(procDataFileIDs, 1)           % Loop through the list of ProcData files
    ...
    clear LH_Rhodamine RH_Rhodamine LH_ElectrodeRhodamine RH_ElectrodeRhodamine LH_GCaMP7s RH_GCaMP7s LH_ElectrodeGCaMP7s RH_ElectrodeGCaMP7s BinTimes WhiskerAcceleration HeartRate DopplerFlow
    ...
    clear cellLH_Rhodamine cellRH_Rhodamine cellLH_ElectrodeRhodamine cellRH_ElectrodeRhodamine cellLH_GCaMP7s cellRH_GCaMP7s cellLH_ElectrodeGCaMP7s cellRH_ElectrodeGCaMP7s cellBinTimes cellWhiskerAcceleration cellHeartRate cellDopplerFlow
    ...
    clear mat2CellLH_Rhodamine mat2CellRH_Rhodamine mat2CellLH_ElectrodeRhodamine mat2CellRH_ElectrodeRhodamine mat2CellLH_GCaMP7s mat2CellRH_GCaMP7s mat2CellLH_ElectrodeGCaMP7s mat2CellRH_ElectrodeGCaMP7s mat2CellBinTimes mat2CellWhiskerAcceleration mat2CellHeartRate mat2CellDopplerFlow
    ...
    clear matLH_Rhodamine matRH_Rhodamine matLH_ElectrodeRhodamine matRH_ElectrodeRhodamine matLH_GCaMP7s matRH_GCaMP7s matLH_ElectrodeGCaMP7s matRH_ElectrodeGCaMP7s matBinTimes matWhiskerAcceleration matHeartRate matDopplerFlow
...
%             %
            ...
            LH_GCaMP7s{indexCount, 1} = ProcData.sleep.parameters.GCaMP7s.LH{fixedSleepIndex(indexCount), 1};
            RH_GCaMP7s{indexCount, 1} = ProcData.sleep.parameters.GCaMP7s.RH{fixedSleepIndex(indexCount), 1};   
...
%                         cellHip_specGammaPower = {arrayHip_specGammaPower};
            ...
            % GCaMP7s
            matLH_GCaMP7s = cell2mat(LH_GCaMP7s);
            arrayLH_GCaMP7s = reshape(matLH_GCaMP7s', [1, size(matLH_GCaMP7s, 2)*size(matLH_GCaMP7s, 1)]);
            cellLH_GCaMP7s = {arrayLH_GCaMP7s};
            
            matRH_GCaMP7s = cell2mat(RH_GCaMP7s);
            arrayRH_GCaMP7s = reshape(matRH_GCaMP7s', [1, size(matRH_GCaMP7s, 2)*size(matRH_GCaMP7s, 1)]);
            cellRH_GCaMP7s = {arrayRH_GCaMP7s};        
...
%                                 mat2CellHip_specGammaPower{matCounter, 1} = Hip_specGammaPower(convertedMat2Cell{matCounter, 1});
                ...
                mat2CellLH_GCaMP7s{matCounter, 1} = LH_GCaMP7s(convertedMat2Cell{matCounter, 1});
                mat2CellRH_GCaMP7s{matCounter, 1} = RH_GCaMP7s(convertedMat2Cell{matCounter, 1});
...
%                                 cellHip_specGammaPower{cellCounter, 1} = arrayHip_specGammaPower;
                ...
                matLH_GCaMP7s = cell2mat(mat2CellLH_GCaMP7s{cellCounter, 1});
                arrayLH_GCaMP7s = reshape(matLH_GCaMP7s', [1, size(matLH_GCaMP7s, 2)*size(matLH_GCaMP7s, 1)]);
                cellLH_GCaMP7s{cellCounter, 1} = arrayLH_GCaMP7s;
                
                matRH_GCaMP7s = cell2mat(mat2CellRH_GCaMP7s{cellCounter, 1});
                arrayRH_GCaMP7s = reshape(matRH_GCaMP7s', [1, size(matRH_GCaMP7s, 2)*size(matRH_GCaMP7s, 1)]);
                cellRH_GCaMP7s{cellCounter, 1} = arrayRH_GCaMP7s;
...
%                                 SleepData.(modelName).NREM.data.hippocampus.specGammaBandPower{cellLength, 1} = cellHip_specGammaPower{1, 1};
                ...
                SleepData.(modelName).NREM.data.GCaMP7s.LH{cellLength, 1} = cellLH_GCaMP7s{1, 1};
                SleepData.(modelName).NREM.data.GCaMP7s.RH{cellLength, 1} = cellRH_GCaMP7s{1, 1};
...
%                                 SleepData.(modelName).NREM.data.hippocampus.specGammaBandPower{size(SleepData.(modelName).NREM.data.hippocampus.specGammaBandPower, 1) + 1, 1} = cellHip_specGammaPower{cellLength, 1};
                ...
                SleepData.(modelName).NREM.data.GCaMP7s.LH{size(SleepData.(modelName).NREM.data.GCaMP7s.LH, 1) + 1, 1} = cellLH_GCaMP7s{cellLength, 1};
                SleepData.(modelName).NREM.data.GCaMP7s.RH{size(SleepData.(modelName).NREM.data.GCaMP7s.RH, 1) + 1, 1} = cellRH_GCaMP7s{cellLength, 1};
...
%                 %
                ...
                SleepData.(modelName).REM.data.GCaMP7s.LH{cellLength, 1} = cellLH_GCaMP7s{1, 1};
                SleepData.(modelName).REM.data.GCaMP7s.RH{cellLength, 1} = cellRH_GCaMP7s{1, 1};
...
%                 
                ...
                SleepData.(modelName).REM.data.GCaMP7s.LH{size(SleepData.(modelName).REM.data.GCaMP7s.LH, 1) + 1, 1} = cellLH_GCaMP7s{cellLength, 1};
                SleepData.(modelName).REM.data.GCaMP7s.RH{size(SleepData.(modelName).REM.data.GCaMP7s.RH, 1) + 1, 1} = cellRH_GCaMP7s{cellLength, 1};
