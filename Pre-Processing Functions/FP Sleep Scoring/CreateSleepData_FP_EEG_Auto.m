function [SleepData] = CreateSleepData_FP_EEG_Auto(startingDirectory,trainingDirectory,baselineDirectory,NREMsleepTime,REMsleepTime,modelName,SleepData)
...
% sleepType = {'NREM','REM'};
dataTypes = {'EEG_LH','EMG','Rhodamine','GFP','Pupil'};
...
for a = 1:size(procDataFileIDs, 1)           % Loop through the list of ProcData files
    ...
    if isempty(sleepIndex)  % If sleepIndex is empty, skip this file
        ...
        if isempty(indexBreaks)   % If there is only one period of sleep in this file and not multiple
            ...
            splitCounter = 1:length(PData.EEG_LH.deltaBandPower);
            ...
            for cellCounter = 1:length(mat2CellData.EEG_LH.deltaBandPower)
                for dn = 1:length(dataTypes)
                    dataType = char(dataTypes(dn));
                    if strcmp(dataType,'EMG') == true
                        subDataTypes = {'emg','power'};
        ...
        if isfield(SleepData,(modelName)) == false  % If the structure is empty, we need a special case to format the struct properly
            for cellLength = 1:size(cellData.EEG_LH.deltaBandPower, 2)   % Loop through however many sleep epochs this file has
                for dn = 1:length(dataTypes)
                    dataType = char(dataTypes(dn));
                    if strcmp(dataType,'EMG') == true
                        subDataTypes = {'emg','power'};
            ...
            for cellLength = 1:size(cellData.EEG_LH.deltaBandPower, 1)   % Loop through however many sleep epochs this file has
