% function [figHandle,ax1,ax2,ax3,ax4,ax6,ax7] = Figures_Sleep_FP_GRABNE_CheckSleep(procDataFileID,saveFigs)
...
% Ach data
ax5 = subplot(8,1,5);
p8 = plot((1:length(filtAch_GFP))/ProcData.notes.dsFs,filtAch_GFP,'color','b','LineWidth',1);
ylabel('BF GCaMP7s')
...
% Rhodamine
ax6 = subplot(8,1,6);
p10 = plot((1:length(filtAch_Rhodamine))/ProcData.notes.dsFs,filtAch_Rhodamine,'color','r','LineWidth',1);
ylabel('\Delta F/F')
yyaxis right
p11 = plot((1:length(filtNE_Rhodamine))/ProcData.notes.dsFs,filtNE_Rhodamine,'color','k','LineWidth',1);
ylabel('\Delta F/F')
legend([p10,p11],'BF Rhodamine','S1 Rhodamine')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'Xticklabel',[])
set(gca,'box','off')
axis tight
