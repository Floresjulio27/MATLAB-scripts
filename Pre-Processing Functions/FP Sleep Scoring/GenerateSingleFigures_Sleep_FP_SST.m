function [figHandle,ax1,ax2,ax3,ax4,ax6] = GenerateSingleFigures_Sleep_FP_SST(procDataFileID,saveFigs)
...
if (isempty(LPad_Yvals) == 1) && (isempty(RPad_Yvals) == 1) && (isempty(Aud_Yvals) == 1)
    legend([p8,s1,s2],'SST RH','movement','whisking')
else
    legend([p8,s1,s2,s3,s4,s5],'SST RH','movement','whisking','LSol','RSol','AudSol')
end
