function [figHandle,ax1,ax2,ax3,ax4,ax5,ax6,ax7] = GenerateSingleFigures_MicroArousal_FP_GRABNE(procDataFileID,saveFigs,MALabels,EMGArousalLabels,Notes,GammaArousalLabels)
...
indecesMax = max(filteredpupildiameter);
emg_Yvals = 1.20*max(indecesMax)*ones(size(emgBinarized));
