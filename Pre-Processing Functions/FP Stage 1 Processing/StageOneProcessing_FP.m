...
%     if isempty(fileExist)
        ...
        % try to see if a rawData file already exist
        RawFileId = [trialData.animalID '_' fileID '_RawData'];
        try
            load(RawFileId);
            ...
            RawData.notes.droppedPupilCamFrameIndex = TDMSFile.droppedPupilCamFrameIndex;
            RawData.notes.droppedwhiskCamFrameIndex = TDMSFile.droppedwhiskCamFrameIndex;
            % stimulus parameters
        ...
        if isfield(RawData,'whiskerAngle_long') == false
        ...
        save([RawData.notes.animalID '_' fileID '_RawData'],'RawData','-v7.3')
end
