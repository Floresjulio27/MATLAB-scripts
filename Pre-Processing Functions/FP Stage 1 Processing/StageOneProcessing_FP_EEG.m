% function StageOneProcessing_FP_EEG
...
%     if isempty(fileExist)
        ...
        trialData = ReadInTDMSWhiskerTrials_FP_EEG([fileID '.tdms']);
        % Left, Right ECOG electrodes
        dataRow = strcmp(trialData.data.names,'cortLH');
        EEG_LH = trialData.data.vals(dataRow,:)/str2double(trialData.amplifierGain);
        dataRow = strcmp(trialData.data.names,'cortRH');
        EEG_RH = trialData.data.vals(dataRow,:)/str2double(trialData.amplifierGain);
        ...
        % Data
        RawData.data.EEG_LH_long = EEG_LH;
        RawData.data.EEG_RH_long = EEG_RH;
        ...
        save([trialData.animalID '_' fileID '_RawData'],'RawData','-v7.3')
end
