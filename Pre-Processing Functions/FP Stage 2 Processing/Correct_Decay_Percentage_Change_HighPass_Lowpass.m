...
% A low-pass filter is designed to model the baseline decay (e.g., photobleaching or metabolic clearance).
%A high-pass filter is designed to isolate fast changes in fluorescence (e.g., neural or hemodynamic activity).
%The low-pass filter generates a smoothed baseline (LowPassFitData), which
%acts as F0 (mean fluorescence)
%The high-pass filter generates the fast changes (delta F), which represent deviations from the baseline.
            ...
            HighPassData = filtfilt(Params.sos_high,Params.g_high,RawData); % High pass filtered data. Also represents the absolute changes. Fluoresence - Mean. Delta F. Remember that the cutoff is 1HZ (you did this before though when you smooth it)
