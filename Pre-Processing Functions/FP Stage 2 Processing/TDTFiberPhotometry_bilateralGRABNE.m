...
%{
            CorrectSlope_RH = HemodynamicsConstantCalculation(isosCorrect_RH,[Params.savepath '.mat'],'RH'); % calculate the slope to correct for CBV
            
            disp(['The hemodynamic correction slope is = ' num2str(CorrectSlope_RH)]); disp(' ')
            Params.correctionConstant_calculated_RH = CorrectSlope_RH;

            ChangeSlope = input('Do you want to change the slope (y/n)? ','s'); disp(' ')
            if ChangeSlope == ("y")
                CorrectSlope_RH = input('What is the new slope? '); %-0.23;
            end
            Params.correctionConstant_used_RH = CorrectSlope_RH;

            CorrectSlope_LH = HemodynamicsConstantCalculation(isosCorrect_LH,[Params.savepath '.mat'],'LH'); % calculate the slope to correct for CBV
            
            disp(['The hemodynamic correction slope is = ' num2str(CorrectSlope_LH)]); disp(' ')
            Params.correctionConstant_calculated_LH = CorrectSlope_LH;

            ChangeSlope = input('Do you want to change the slope (y/n)? ','s'); disp(' ')
            if ChangeSlope == ("y")
                CorrectSlope_LH = input('What is the new slope? '); %-0.23;
            end

            Params.correctionConstant_used_LH = CorrectSlope_LH;
...
%             Params.correctionConstant = -0.23; % used from Kyles Data -0.23
            ...
            CBVCorrection_LH.F465 = CBVCorrection_LH.F465-(CorrectSlope_LH*CBVCorrection_LH.F560);
            CBVCorrection_RH.F465 = CBVCorrection_RH.F465-(CorrectSlope_RH*CBVCorrection_RH.F560);
