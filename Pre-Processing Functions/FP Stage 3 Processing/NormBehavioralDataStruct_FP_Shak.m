function [DataStruct] = NormBehavioralDataStruct_FP_Shak(DataStruct,RestingBaselines,baselineType)
...
for dT = 1:length(dataTypes)
    ...
    if strcmp(dataType,'TRITC') == false && strcmp(dataType,'GCaMP7s') == false % normalize data for everything else
    ...
    elseif strcmp(dataType,'TRITC') == true || strcmp(dataType,'GCaMP7s') == true %% Zscore the data for fiber photometry
        hemisphereDataTypes = fieldnames(DataStruct.(dataType));       
        for hDT = 1:length(hemisphereDataTypes)
            hemDataType = char(hemisphereDataTypes(hDT));           
            if isfield(DataStruct.(dataType).(hemDataType),'whisk')
