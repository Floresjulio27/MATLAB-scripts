function [RestingBaselines] = CalculateSpectrogramBaselines_FP_TimeCindex(animal,neuralDataTypes,trialDuration_sec,RestingBaselines,baselineType)
...
%________________________________________________________________________________________________________________________
%
%   Purpose: Uses the resting time indices to extract the average resting power in each frequency bin during periods of
...
for a = 1:length(neuralDataTypes)
    ...
    for d = 1:length(restFileList)
        ...
        strDay = ConvertDate_FP(fileID(1:6));
        ...
        for e = 1:length(RestingBaselines.(baselineType).baselineFileInfo.fileIDs)
            ...
            if strcmp(fileID,restFileID)
                ...
                tolerance = 1e-3; % You can adjust this tolerance based on your data precision
                
                % 1 second spectrogram conditions and indexing
                if startTime1C >= 0.5 && (startTime1C + restDuration1C) <= (trialDuration_sec - 0.5)
                    startTime1C_index = find(abs(T1C - startTime1C) < tolerance);
                    if isempty(startTime1C_index)
                        warning('startTime1C value not found in T1C within tolerance.');
                        continue; % Skip if no match is found
                    else
                        startTime1C_index = startTime1C_index(1);
                        restDuration1C_Index = round(restDuration1C * binSize1C) - 1;  % Ensure it's rounded to an integer
                        % Check if the indices are within bounds
                        if (startTime1C_index + restDuration1C_Index) <= size(S1C_data, 2)
                            ...
                            warning('Index exceeds the dimensions of S1C_data. Skipping this rest segment.');
                        end
                    end
                end
                ...
                if startTime1B >= 0.5 && (startTime1B + restDuration1B) <= (trialDuration_sec - 0.5)
                    startTime1B_index = find(abs(T1B - startTime1B) < tolerance);
                    if isempty(startTime1B_index)
                        warning('startTime1B value not found in T1B within tolerance.');
                        continue;
                    else
                        startTime1B_index = startTime1B_index(1);
                        restDuration1B_Index = round(restDuration1B * binSize1B) - 1;
                        if (startTime1B_index + restDuration1B_Index) <= size(S1B_data, 2)
                            S1B_single_rest = S1B_data(:,(startTime1B_index:(startTime1B_index + restDuration1B_Index)));
                            S1B_trialRest = [S1B_single_rest,S1B_trialRest];
                        else
                            warning('Index exceeds the dimensions of S1B_data. Skipping this rest segment.');
                        end
                    end
                end
                
                % 5 second spectrogram conditions and indexing
                if startTime5A >= 5 && (startTime5A + restDuration5A) <= (trialDuration_sec - 5)
                    [~,startTime5A_index] = min(abs(T5A - startTime5A));
                    restDuration5A_Index = floor(restDuration5A*binSize5A) - 1;
                    if (startTime5A_index + restDuration5A_Index) <= size(S5C_data, 2)
                        ...
                        warning('Index exceeds the dimensions of S5C_data. Skipping this rest segment.');
                    end
                end
            end
        end
    ...
    uniqueDays = GetUniqueDays_FP(RestingBaselines.(baselineType).baselineFileInfo.fileIDs);   
    for f = 1:length(uniqueDays)
        g = 1;
        for field = 1:length(fields)
            if strcmp(fields{field}(7:12), uniqueDays{f})
                stringDay = ConvertDate_FP(uniqueDays{f});
