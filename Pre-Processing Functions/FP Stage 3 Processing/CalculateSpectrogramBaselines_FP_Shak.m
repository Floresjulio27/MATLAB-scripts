function [RestingBaselines] = CalculateSpectrogramBaselines_FP_Shak(animal,neuralDataTypes,trialDuration_sec,RestingBaselines,baselineType)
...
for a = 1:length(neuralDataTypes)
    ...
    for d = 1:length(restFileList)
        ...
        for e = 1:length(RestingBaselines.(baselineType).baselineFileInfo.fileIDs)
            ...
            if strcmp(fileID,restFileID)
                ...
                if startTime5A >= 5 && (startTime5A + restDuration5A) <= (trialDuration_sec - 5) % I have changed 2.5 with 5
    ...
    for h = 1:length(dayFields)
        ...
        RestingBaselines.Spectrograms.(neuralDataType).oneSecC.(dayFields{h}).mean = mean(dayVals1C,2);
        RestingBaselines.Spectrograms.(neuralDataType).oneSecC.(dayFields{h}).std = std(dayVals1C,2);

        RestingBaselines.Spectrograms.(neuralDataType).oneSecB.(dayFields{h}).mean = mean(dayVals1B,2);
        RestingBaselines.Spectrograms.(neuralDataType).oneSecB.(dayFields{h}).std = std(dayVals1B,2);

        RestingBaselines.Spectrograms.(neuralDataType).fiveSecA.(dayFields{h}).mean = mean(dayVals5A,2);
        RestingBaselines.Spectrograms.(neuralDataType).fiveSecA.(dayFields{h}).std = std(dayVals5A,2);
