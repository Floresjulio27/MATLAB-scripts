function StageThreeProcessing_FP_EEG()
...
dataTypes = {'Rhodamine','GFP','EEG_LH','EEG_RH','EMG','Pupil'};
...
[RestData] = ExtractRestingData_FP_EEG(procDataFileIDs,dataTypes);
...
[EventData] = ExtractEventTriggeredData_FP_EEG(procDataFileIDs,dataTypes);
...
[RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_FP_EEG();
...
[RestData] = ExtractRestingData_FP_Pupil_EEG(procDataFileIDs,dataTypes);
% add pupil area to EventData.mat
[EventData] = ExtractEventTriggeredData_FP_Pupil_EEG(procDataFileIDs,dataTypes);
%% BLOCK PURPOSE: [5] Manually select files for custom baseline calculation
disp('Analyzing Block [5] Manually select files for custom baseline calculation.'); disp(' ')
[RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_FP_EEG(); % no need to redo the selection
%% BLOCK PURPOSE: [7] Normalize RestData and EventData structures by the resting baseline
...
disp('Analyzing Block [7] Normalizing RestData and EventData structures by the resting baseline.'); disp(' ')
...
for bb = 1:size(procDataFileIDs,1)
    ...
    [figHandle] = GenerateSingleFigures_FP_EEG(procDataFileID,saveFigs);
    close(figHandle)
end
