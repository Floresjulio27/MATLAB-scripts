...
indecesMax = max([filtAch_Rhodamine;filtAch_Rhodamine;filtAch_GFP;filtAch_GFP]);
...
indecesMax = max(filteredpupilarea);
...
% Pupil area and behavioral indeces
ax3 = subplot(7,1,3);
...
ylabel('\delta F/F (Z)')
legend([p7,p5],'GRAB Ach','mScarlet')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'TickLength',[0,0])
set(gca,'Xticklabel',[])
set(gca,'box','off')
...
ylabel('\delta F/F (Z)')
yyaxis right
p6 = plot((1:length(filtNE_Rhodamine))/ProcData.notes.dsFs,filtNE_Rhodamine,'color',colors('dark candy apple red'),'LineWidth',1);
ylabel('\delta F/F (Z)')
legend([s1,s2,s3,p8,p6],'LSol','RSol','AudSol','GRAB NE', 'mScarlet')
xlim([0,ProcData.notes.trialDuration_sec])
set(gca,'TickLength',[0,0])
set(gca,'Xticklabel',[])
set(gca,'box','off')
