function [RestingBaselines] = CalculateManualRestingBaselinesTimeIndeces_FP_GRABNE(fiberType)
...
for a = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData,'manualBaselineInfo') == false %|| isfield(ProcData,'manualBaselineInfo') == true
        saveFigs = 'y';
        b = false;
        while b == false
            ...
            if fiberType == 1
            [singleTrialFig] = GenerateSingleFigures_FP_GRABNE_SingleFiber(procDataFileID,saveFigs);
            elseif fiberType == 2
...
for e = 1:length(dataTypes)
    ...
    for f = 1:length(subDataTypes)
        ...
        uniqueDays = GetUniqueDays_FP(RestData.(dataType).(subDataType).fileIDs);
        ...
        for n = 1:length(filtFileIDs)
            ...
            if (eventTime-5) >= sTime && (eventTime+10) <= eTime % changed from 2 to 5
        ...
        for p = 1:length(uniqueDays)
            ...
            for r = 1:length(finalEventFileIDs)
                ...
                uniqueDate{p,1} = ConvertDate_FP(uniqueDays{p,1});
        ...
        for t = 1:length(uniqueDays)
            RestingBaselines.manualSelection.(dataType).(subDataType).(uniqueDate{t,1}).mean = mean(tempDataMeans{t,1});
            RestingBaselines.manualSelection.(dataType).(subDataType).(uniqueDate{t,1}).std = std(tempDataMeans{t,1});
        end
    end
end
% save results
