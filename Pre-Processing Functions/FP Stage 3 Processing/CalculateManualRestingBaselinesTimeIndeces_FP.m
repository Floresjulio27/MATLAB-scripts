...
for a = 1:size(procDataFileIDs,1)
    ...
    if isfield(ProcData,'manualBaselineInfo') == false
        ...
        RedoDecision = input(['Manual basline selection already exist for ' procDataFileID ' . Do you want to perform manual resting baseline calculation again? (y/n): '], 's'); disp(' ')
            if strcmp(RedoDecision,'y')
                saveFigs = 'n';
                b = false;
                while b == false
                    % load a figure with the data to visualize which periods are rest. Note that this data is, by default, normalized
