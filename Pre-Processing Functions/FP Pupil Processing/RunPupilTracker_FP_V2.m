function [] = RunPupilTracker_FP_V2(rawDataFileIDs)
...
%________________________________________________________________________________________________________________________
% This function save the pupil data to RawData instead of ProcData which
% was wriiten by Kevin. 
% updated by Shakhawat
% this version works on longer duration files compared to 15 mintues file. 
...
if isfield(PupilData,'firstFileOfDay') == false
    ...
    [~,fileDates,~] = GetFileInfo_FP_Shak(rawDataFileIDs);
    [uniqueDays,~,DayID] = GetUniqueDays_FP(fileDates);
    for aa = 1:length(uniqueDays)
        strDay = ConvertDate_FP(uniqueDays{aa,1});
        FileInd = DayID == aa;
        dayFilenames.(strDay) = rawDataFileIDs(FileInd,:);
    end
...
%             tic;
            ...
            imageHeight = RawData.notes.pupilCamPixelHeight; %#ok<*NODEF>
...
%         tic;
        ...
        if RawData.notes.pupilCamPixelHeight > 200
            newROI = [1,1,199,199];
            newROIFig = figure;
...
for cc = 1:size(rawDataFileIDs,1)
    clearvars -except rawDataFileIDs PupilData cc
    try
        procDataFileID = rawDataFileIDs{cc,1};
    catch
        procDataFileID = rawDataFileIDs(cc,:);
    end
    load(char(procDataFileID))
    disp(['Checking pupil tracking status of file (' num2str(cc) '/' num2str(size(rawDataFileIDs,1)) ')']); disp(' ')
    if isfield(RawData.data,'Pupil') == false
%     if isfield(RawData.data.Pupil,'algorithmUpdate') == false
%         RawData.data.Pupil.originalPupilArea = RawData.data.Pupil.pupilArea;
%         RawData.data.Pupil.originalBlinkInds = RawData.data.Pupil.blinkInds;
tic;
        ...
        imageHeight = RawData.notes.pupilCamPixelHeight; % how many pixels tall is the frame
        imageWidth = RawData.notes.pupilCamPixelWidth; % how many pixels wide is the frame
        samplingRate = RawData.notes.pupilCamSamplingRate; % pupil acquisition rate (Hz)
        pixelsPerFrame = imageWidth*imageHeight;
        skippedPixels = pixelsPerFrame;
        ...
        RawData.data.Pupil.firstFrame = imageStack(:,:,2);
toc;
...
%         overlay(1:size(imageStack,1),1:size(imageStack,2),3,1:size(imageStack,3)) = NaN;
        ...
        if endIdx <= MaxFrames
            imageFrames = gpuArray(imageStack(:,:,startIdx:endIdx));
        elseif endIdx > MaxFrames
            imageFrames = gpuArray(imageStack(:,:,startIdx:end));    
        end
        % pre-allocate empty structures
        pupilArea_temp(1:size(imageFrames,3)) = NaN; % area of pupil
        pupilMajor_temp(1:size(imageFrames,3)) = NaN; % length of major axis of pupil
        pupilMinor_temp(1:size(imageFrames,3)) = NaN; % length of minor axis of pupil
        pupilCentroid_temp(1:size(imageFrames,3),2) = NaN; % center of pupil
        pupilBoundary_temp(1:size(imageFrames,1),1:size(imageFrames,2),1:size(imageFrames,3)) = NaN;
%         imageFrames = gpuArray(imageStack);
        roiInt(1:size(imageFrames,3)) = NaN;
        roiInt = gpuArray(roiInt);
        correctedFlag = false;
        lastFrameOK = 1;
        for frameNum = 1:size(imageFrames,3)
            ...
            if ~isempty(areaFilled) % is an pupil identified
                ...
                pupilArea_temp(frameNum) = areaFilled.FilledArea;
                pupilMajor_temp(frameNum) = areaFilled.MajorAxisLength;
                pupilMinor_temp(frameNum) = areaFilled.MinorAxisLength;
                pupilCentroid_temp(frameNum,:) = areaFilled.Centroid;
                % overlay comsumes too much space and most of the time not
                % used for any calculations
                pupilBoundary_temp(:,:,frameNum) = fillPupil;
...
%                 if size(holdMat,3) == 1
%                     overlay_temp(:,:,:,frameNum) = repmat(holdMat,1,1,3);
%                 else
%                     overlay_temp(:,:,:,frameNum) = holdMat; % this is the measured pupil overlayed on the movie
%                 end
            else
                pupilArea_temp(frameNum) = NaN;
                pupilMajor_temp(frameNum) = NaN;
                pupilMinor_temp(frameNum) = NaN;
                pupilCentroid_temp(frameNum,:) = NaN;
                pupilBoundary_temp(:,:,frameNum) = fillPupil;
...
%                 end
        ...
        if endIdx <= MaxFrames
        pupilArea(:,startIdx:endIdx) = pupilArea_temp;
        pupilMajor(:,startIdx:endIdx) = pupilMajor_temp;
        pupilMinor(:,startIdx:endIdx) = pupilMinor_temp;
        pupilCentroid(startIdx:endIdx,:) = pupilCentroid_temp;
        pupilBoundary(:,:,startIdx:endIdx) = pupilBoundary_temp;
%         overlay(:,:,:,startIdx:endIdx) = overlay_temp;
        elseif endIdx > MaxFrames
        pupilArea(:,startIdx:end) = pupilArea_temp;
        pupilMajor(:,startIdx:end) = pupilMajor_temp;
        pupilMinor(:,startIdx:end) = pupilMinor_temp;
        pupilCentroid(startIdx:end,:) = pupilCentroid_temp;
        pupilBoundary(:,:,startIdx:end) = pupilBoundary_temp;
%         overlay(:,:,:,startIdx:end) = overlay_temp;
        end 

        clear pupilArea_temp pupilMajor_temp pupilMinor_temp pupilCentroid_temp pupilBoundary_temp overlay_temp
        end
        ...
        RawData.data.Pupil.pupilMinor = pupilMinor;
        RawData.data.Pupil.pupilCentroid = pupilCentroid;
        RawData.data.Pupil.eyeROI = eyeROI;
        RawData.data.Pupil.roiIntensity = gather(roiInt);
...
%         RawData.data.Pupil.blinkFrames = overlay(:,:,:,blinks);
        ...
        testPupilAreaA = RawData.data.Pupil.rawPupilArea;
        patchLength = [];
        ...
        try
            RawData.data.Pupil.patchedPupilAreaA = fillmissing(testPupilAreaA,'movmedian',max(patchLength)*2);
        catch
        ...
        % fill from start:ending edges of rapid pupil fluctuations that weren't NaN
        testPupilAreaB = RawData.data.Pupil.patchedPupilAreaA;
        ...
        RawData.data.Pupil.patchedPupilAreaB = testPupilAreaB;
        RawData.data.Pupil.pupilArea = RawData.data.Pupil.patchedPupilAreaB;
        RawData.data.Pupil.algorithmUpdate = true;
