...
%________________________________________________________________________________________________________________________
function AccessoryPupilUpdate_FP(rawDataFileIDs)
% zap;
...
for qq = 1:size(rawDataFileIDs,1)
    ...
    load(rawDataFileID)
    if isfield(RawData.data.Pupil,'accessoryPatch') == false
        disp(['Updating accessory pupil data types of file ' num2str(qq) '/' num2str(size(rawDataFileIDs,1))]); disp(' ')
%         [animalID,~,fileID] = GetFileInfo_JNeurosci2022(rawDataFileID);
        ...
        RawData.data.Pupil.patchMinorAxis = patchedMinorAxis;
        RawData.data.Pupil.patchMajorAxis = patchedMajorAxis;
        RawData.data.Pupil.patchCentroidX = patchedCentroidX;
        RawData.data.Pupil.patchCentroidY = patchedCentroidY;
        % save data
        RawData.data.Pupil.accessoryPatch = 'y';
        save(rawDataFileID,'RawData','-v7.3')
    else
        disp(['Data types already updated for ' num2str(qq) '/' num2str(size(rawDataFileIDs,1))]); disp(' ')
    end
end
