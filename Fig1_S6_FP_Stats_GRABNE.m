...
for aa = 1:length(FP_animalIDs)
    ...
    for bb = 1:length(behavFields)
        ...
        for cc = 1:length(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndAch)
            data.Rhodamine.(animalID).(behavField).meanAch(cc,1) = max(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndAch{cc,1});
        end
        for cc = 1:length(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndNE)
            data.Rhodamine.(animalID).(behavField).meanNE(cc,1) = max(AnalysisResults.(animalID).MeanRhodamine.(behavField).Rhodamine.IndNE{cc,1});
        end
    end
end
...
for aa = 1:length(FP_animalIDs)
    ...
    for bb = 1:length(behavFields)
        ...
        for cc = 1:length(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndAch)
            data.GFP.(animalID).(behavField).meanAch(cc,1) = max(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndAch{cc,1});
        end
        for cc = 1:length(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndNE)
            data.GFP.(animalID).(behavField).meanNE(cc,1) = max(AnalysisResults.(animalID).MeanGFP.(behavField).GFP.IndNE{cc,1});
        end
    end
end
% put data into arrays and prep for stats
...
% Rhodamine_meanTable = table('Size',[size(tableSize,1),4],'VariableTypes',{'string','string','string','double'},'VariableNames',{'Mouse','Hemisphere','Behavior','Max'});
...
% Rhodamine_meanFitFormula = 'Max ~ 1 + Behavior + (1|Mouse) + (1|Mouse:Hemisphere)';
...
% GFP_meanTable = table('Size',[size(tableSize,1),4],'VariableTypes',{'string','string','string','double'},'VariableNames',{'Mouse','Hemisphere','Behavior','Max'});
...
title({'Max Zscored \DeltaRhodamine Ach'})
ylabel('Max  \DeltaRhodamine Ach')
...
title({'Max Zscored \DeltaRhodamine NE'})
ylabel('Max  \DeltaRhodamine NE')
legend([s3,s1,s2,s4,s5],'Rest','Whisk','Stim','NREM','REM','Location','best')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
...
title({'Max Zscored \DeltaGFP Ach'})
ylabel('Max Zscored \DeltaGFP Ach')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
axis tight
...
title({'Max Zscored \DeltaGFP NE'})
ylabel('Max Zscored \DeltaGFP NE')
set(gca,'xtick',[])
set(gca,'xticklabel',[])
axis square
axis tight
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure,[dirpath animalID '_Fig1-S6-Stats']);
    set(summaryFigure,'PaperPositionMode','auto');
    print('-painters','-dpdf','-bestfit',[dirpath animalID '_Fig1-S6-Stats'])
    close 
    %% Text diary
