...
% function GenerateFigures_Proposal_Modulators()
...
NE_GFP = resample(ProcData.data.GFP.P_NE,resample_rate,ProcData.notes.dsFs);
NE_GFP1 = medfilt1(NE_GFP,20);
filtNE_GFP = filtfilt(sos2,g2,NE_GFP1);
filtNE_GFP = sgolayfilt(filtNE_GFP,3,15);

% ACh data
ACh_GFP = resample(ProcData.data.GFP.P_ACh,resample_rate,ProcData.notes.dsFs);
ACh_GFP1 = medfilt1(ACh_GFP,20);
filtACh_GFP = filtfilt(sos2,g2,ACh_GFP1);
filtACh_GFP = sgolayfilt(filtACh_GFP,3,15);
% CBV data
NE_CBV = resample(ProcData.data.CBV.P_NE,resample_rate,ProcData.notes.dsFs);
NE_CBV1 = medfilt1(NE_CBV,20);
filtNE_CBV = filtfilt(sos2,g2,NE_CBV1); 
filtNE_CBV = sgolayfilt(filtNE_CBV,3,15);

ACh_CBV = resample(ProcData.data.CBV.P_ACh,resample_rate,ProcData.notes.dsFs);
ACh_CBV1 = medfilt1(ACh_CBV,20);
filtACh_CBV = filtfilt(sos2,g2,ACh_CBV1); 
filtACh_CBV = sgolayfilt(filtACh_CBV,3,15);
...
plot((1:length(filtNE_GFP))/(resample_rate*60),filtNE_GFP,'color',[0.4660 0.6740 0.1880],'LineWidth',1);
ylabel('\Delta F/F (%)')
hold on
ylim([-8 6])
yyaxis right
...
plot((1:length(filtNE_CBV))/(resample_rate*60),filtNE_CBV,'color',[ 0.6350 0.0780 0.1840],'LineWidth',1);
ylim([-.5 .5])
legend('NE','CBV')
ylabel('\Delta F/F (%)')
...
% ax6 = subplot(9,1,[7:9]);
% % % yyaxis right
% plot((1:length(filtACh_CBV))/(resample_rate),filtACh_CBV,'color',[0.1350 0.0780 0.6840],'LineWidth',1);
% ylim([-3.5 4])
% ylabel('\Delta F/F (Z)')
% legend('CBV')
% hold on
...
% % yyaxis right
% 
% plot((1:length(filtNE_CBV))/(resample_rate),filtNE_CBV,'color',[ 0.6350 0.0780 0.1840],'LineWidth',1);
% ylim([-3.5 4])
% ylabel('\Delta F/F (Z)')
