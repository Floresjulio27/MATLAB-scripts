function [AnalysisResults] = AnalyzeMeanGFP_FP_GRABNE_Sleep(animalID,rootFolder,AnalysisResults,firstHrs)
...
%________________________________________________________________________________________________________________________
%
%   Purpose: Analyze the hemodynamic signal [GFP] during different arousal states
%________________________________________________________________________________________________________________________
%% function parameters
modelType = 'Manual';
...
for nn = 1:size(ACh_ProcWhiskData,1)
    ACh_whiskGFPMean(nn,1) = mean(ACh_ProcWhiskData(nn,params.Offset*samplingRate:params.minTime.Whisk*samplingRate),2); % mean
...
if firstHrs == "true"
    [AChremData,nremFileIDs,~] = NRemoveStimSleepData_IOS(animalID,SleepData.(modelType).NREM.data.GFP.P_ACh,SleepData.(modelType).NREM.FileIDs,SleepData.(modelType).NREM.BinTimes);
    [NEremData,~,~] = NRemoveStimSleepData_IOS(animalID,SleepData.(modelType).NREM.data.GFP.P_NE,SleepData.(modelType).NREM.FileIDs,SleepData.(modelType).NREM.BinTimes);
