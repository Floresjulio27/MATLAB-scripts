function [AnalysisResults] = Fig4_FP_Transition_GRABNE_SingleMouse_consolidated_CI(rootFolder,saveFigs,delim,AnalysisResults,firstHrs,FPanimalIDs)
...
% Blood Volume and NE, ACh
    ...
    legend([p2,p3,pq,pC],'NE','ACh','ACh-NE','CBV','Location','northeast','FontSize',5)
        % legend([p1,p4,p2,p3,pq,pC],'CBV LH','CBV RH','NE','ACh','ACh-NE','CBV','Location','northeast','FontSize',5)
    xlim([-30,30])
    title('Awake to NREM')
    set(gca,'box','off')
    ax1.YAxis(1).Color = 'k';
...
if strcmp(saveFigs,'y') == true
    ...
    savefig(summaryFigure_D,[dirpath ManipulationType '_Transition_CBV_vs_GFP_Sleep_CI_ACh_NE']);
    set(summaryFigure_D,'PaperPositionMode','auto');
    print('-painters','-dpdf','-fillpage',[dirpath ManipulationType '_Transition_CBV_vs_GFP_Sleep_CI_ACh_NE'])
end
close
